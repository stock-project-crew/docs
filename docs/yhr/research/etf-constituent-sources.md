...
>
> 작성일: 2026-06-28 (최종수정: 2026-07-07 — pykrx 1.2.8 + KRX 로그인 재검증 반영, §0-3)
...
| **pykrx / KRX** | ❌ **차단(JS 게이트)** *(1.0.51 기준 — **1.2.8에서 무효화, §0-3 참조**)* | `get_etf_portfolio_deposit_file` 빈 DF, `get_etf_ticker_list()` `IndexError`. 원인: `getJsonData.cmd`가 **모든 bld**(전종목 시세 `MDCSTAT01501` 포함)에서 **`LOGOUT`(HTTP 400)**. KRX 첫 페이지는 474B JS 부트스트랩(`/WEB-APP/webponent/ci/agent.js`)뿐 — JS 실행 없이는 유효 세션 미발급. → **pykrx 1.2.8은 KRX 계정 로그인 방식으로 바뀌어, env 자격증명 주입 시 순수 requests로 정상 동작(§0-3).** |
...
### §0-3 재검증 (2026-07-07) — pykrx **1.2.8** + KRX 계정 로그인으로 **KRX 백엔드 전량 정상화** ✅
> ⚠️ **중대 갱신:** §0/§0-근본원인/§1-1의 "**전량 차단(pykrx 1.0.51, agent.js JS 게이트)**" 결론은
> **최신 pykrx 1.2.8에서 무효화됨.** 1.2.8은 KRX 백엔드 함수 호출 시 **KRX 계정 로그인(env `KRX_ID`/`KRX_PW`)** 을
> 요구하며, **자격증명을 주입하면 순수 파이썬(requests 기반, 헤드리스 브라우저 없이) 만으로 전 함수가 동작**한다.
> 실행 환경: macOS + 프로젝트 `.venv`(pykrx 1.2.8). 읽기 전용 조회.

**동작 변화 요약**
- **1.0.51(1차, 2026-06-28~29)**: 로그인 개념 없음. KRX 백엔드가 `agent.js` JS 게이트로 전량 `LOGOUT` 차단.
- **1.2.8(2026-07-07)**: KRX 백엔드가 **KRX 웹 로그인 세션**을 사용하도록 변경. env 미설정 시
  `"KRX 로그인 실패: KRX_ID 또는 KRX_PW 환경 변수가 설정되지 않았습니다"`로 실패, **설정 시 정상**.
  (Naver 백엔드 `get_market_ohlcv`는 1.0.51/1.2.8 모두 로그인 무관하게 정상.)

**실측 결과 (env 자격증명 주입 후, 전 항목 ✅)**

| 호출 | 백엔드 | env 없음 | **KRX 로그인 후** |
|------|:---:|:---:|:---:|
| `get_index_ohlcv` KOSPI(1001) / KOSDAQ(2001) | KRX | ❌ `KeyError:'지수명'`(빈 JSON) | ✅ **각 10행** (시가/고가/저가/종가/거래량/거래대금/상장시가총액) |
| `get_etf_portfolio_deposit_file(069500)` | KRX | ❌ 빈 DF | ✅ **201행** (삼성전자 33.02%·SK하이닉스 30.73% …) |
| `get_etf_ticker_list` | KRX | ❌ `KeyError:'시장'` | ✅ **1,142개** |
| `get_market_ticker_list` | KRX | ❌ 빈 리스트 | ✅ **945개** |
| `get_market_ohlcv(005930)` (대조군) | Naver | ✅ | ✅ **10행** (로그인 무관) |

**로그인 방식·안정성 관찰**
- **KRX 웹 세션 토큰** 발급 방식. import(첫 호출) 시 1회 로그인 후 세션 재사용.
- 관찰된 세션 수명: **발급 → 약 1시간 뒤 만료**(실측: 로그인 23:08 → 만료 00:08). 만료 후 재로그인 필요 추정.
- 소량 조회에서 별도 호출 한도·차단 관찰 없음(대량·고빈도는 미검증).
- ⚠️ **보안 관찰:** pykrx 1.2.8은 로그인 시 **로그인 ID를 stdout에 평문 출력**(`로그인 ID: …`) → 로그 수집 환경에서 ID 노출 주의. 자격증명은 env(또는 gitignore된 `.env`)로만 주입하고 코드·로그·커밋에 남기지 말 것.

**결론 갱신 (이 문서 기준)**
- **① 지수 시계열(벤치마크) · ② ETF 구성종목**은 **KRX 계정 로그인만으로 pykrx 1.2.8에서 확보 가능**(순수 코드, 헤드리스 불필요) — **확정**.
  → §4 국내 결론의 "무료·순수코드·헤드리스불필요를 동시 충족하는 경로 없음"은 **기술적으로는 해소**(pykrx 1.2.8 + KRX 로그인).
- 단 **견고성**은 여전히 사설 스크래핑 라이브러리 의존(KRX가 로그인/엔드포인트를 또 바꾸면 재차 깨질 수 있음 — 1.0.51→1.2.8 자체가 그 사례).
- 🚫 **컴플라이언스는 변함없음:** KRX 공지(2026-06-01)는 **pykrx 등 사설 라이브러리의 (상용) 이용을 금지**(§1-3).
  이번 방식은 **개발/연습 한정**이며 **상업화 시 재검토 필수**(KRX 데이터상품/정식 라이선스로 이관).

---

## 1. 국내 ETF 구성종목 원천

### 1-1. pykrx (Python 라이브러리, KRX 스크래핑) — **1.0.51 실측 실패(§0) → 1.2.8 + KRX 로그인으로 정상화(§0-3)**
...
- ✅ **재검증(§0-3, 2026-07-07)**: **pykrx 1.2.8**은 KRX 백엔드가 **KRX 계정 로그인(env `KRX_ID`/`KRX_PW`)** 방식으로 바뀜.
  자격증명 주입 시 **지수 시계열·ETF 구성종목·목록류 전부 순수 requests(헤드리스 불필요)로 정상 동작** 확정. (Naver 백엔드는 로그인 무관.)
...
- ✅ **갱신(§0-3, 2026-07-07)**: **pykrx 1.2.8 + KRX 계정 로그인**으로 지수 시계열·ETF 구성종목이 **순수 코드(헤드리스 불필요)로 확보 가능**해짐 →
  아래 "**무료·순수코드·견고 셋 동시충족 경로 없음**"의 **기술적 병목은 해소**. 다만 **① 사설 스크래핑 라이브러리 의존(견고성 리스크 잔존) ② KRX 공지상 상용 금지(컴플라이언스)** 로,
  **개발/연습 단계 한정 채택 후보**이며 상업화 시엔 여전히 데이터상품/정식 라이선스 필요. (아래 표에 **A′. pykrx 1.2.8+KRX로그인** 행 추가.)

| 옵션 | 자동화(코드) | UI변경 견고성 | 약관 | 비용 |
|------|:---:|:---:|:---:|:---:|
| 화면 CSV 수동 다운로드 | ✕(사람) | — | 개인 OK | 무료 |
| **A′. pykrx 1.2.8 + KRX 로그인** *(신규, §0-3)* | ◎(순수 requests, 헤드리스 불필요) | 🟡 중간(라이브러리·KRX 로그인 변경 의존) | ⚠️ 상용 금지(공지) | 무료 |
