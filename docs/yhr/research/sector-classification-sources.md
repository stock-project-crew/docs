...
>
> 작성일: 2026-06-29 (최종수정: 2026-07-08 — pykrx 1.2.8 + KRX 로그인으로 KRX 업종분류 확보 실측, §0-2/§2-E)
...
> **KRX/pykrx = `agent.js` JS 게이트, iShares = Akamai 봇매니저** → 둘 다 **순수** requests/curl로 못 뚫음.
> **단, pykrx 1.2.8은 KRX 계정 로그인 방식으로 바뀌어 KRX 업종분류를 순수 코드로 확보 가능해짐 → §0-2 참조.**

## 0-2. 재검증 (2026-07-08) — pykrx **1.2.8** + KRX 로그인으로 **KRX 업종분류 확보 ✅**

> 배경: pykrx 1.2.8은 KRX 백엔드가 **KRX 계정 로그인(env `KRX_ID`/`KRX_PW`)** 방식으로 전환됨([ETF 문서 §0-3] 실측).
> 그 로그인으로 **종목→업종(섹터) 분류**도 순수 코드(헤드리스 불필요)로 얻어지는지 검증. 환경: `.venv`(pykrx 1.2.8), 읽기전용.

| 검증 | 결과 | 관찰 |
|------|------|------|
| **pykrx `get_market_sector_classifications(date, market)`** | ✅ **성공** | KRX 화면 **[12025] 업종별 분류 현황**(bld `MDCSTAT03901`). KOSPI **945행**/KOSDAQ **1,821행**. 컬럼: `종목명·업종명·종가·대비·등락률·시가총액`(index=6자리 종목코드). 한 번 호출로 **시장 전체 유니버스**. |
| **업종 세분도** | 대분류 flat | KOSPI **26개**, KOSDAQ **24개**. 예: `전기·전자 / 화학 / 은행 / 증권 / 보험 / 유통 / 제약 / 운송·창고 / 기계·장비 / IT 서비스 / 통신 / 건설 / 금속 …`. **KRX 표준 업종(대분류)** — GICS/WICS 아님, 하위 세부업종 없음. |
| **분류 개편 주의** | 라벨 변경됨 | pykrx docstring(2022 예시)의 `서비스업/유통업/철강금속`과 **2026 실데이터 라벨이 다름**(KRX가 업종분류 개편) → 실데이터 기준으로 봐야 함. |
| **대안: 업종 지수 역매핑** | ✅ 동작 | `get_index_ticker_list("KOSPI")`에 업종지수 포함(1008 화학·1013 전기전자·1021 금융, KOSPI200 섹터지수 1151~1156). `get_index_portfolio_deposit_file("1008")`→화학 103종목. 독립 2경로. |
| **curl 직접 호출(대조군)** | ❌ `LOGOUT`(400) | 원시 `getJsonData.cmd`(bld MDCSTAT03901)는 **여전히 JS 봇 게이트로 차단**. pykrx의 KRX 로그인 세션을 거쳐야만 통과. |

**함의**
- 국내 섹터를 **KRX 공식 업종**으로, **순수 코드(헤드리스 불필요)·시장 전체·기준일 지원**으로 얻는 경로가 생김 → §2-E "직접 수집 차단"을 **갱신**(아래).
- 단 **(견고성)** 사설 라이브러리+KRX 로그인 의존(KRX가 또 바꾸면 깨질 수 있음, 1.0.51→1.2.8이 그 사례), **(약관)** KRX 공지(2026-06-01)가 **pykrx 상용 이용 금지** → **개발/연습 한정**, 상업화 시 KRX 데이터상품/정식 라이선스 필요.
- **(세분도)** KRX 업종은 대분류(24~26)라 Naver WICS 79업종보다 거침. 세분도가 중요하면 Naver(§2-B), 공식성이 중요하면 KRX(pykrx).
...
### 2-E. KRX 업종분류 — ⚠️ 순수 curl ❌ / **pykrx 1.2.8 + KRX 로그인 ✅**(2026-07-08 갱신)
- **(a) 접근**: `pykrx.stock.get_market_sector_classifications(date, market)` — KRX 화면 [12025] 업종별 분류 현황(bld `MDCSTAT03901`). 종목→업종을 시장 전체·기준일 단위로 반환(§0-2 실측).
- **(b) 무료/한도**: 무료. KRX 계정 로그인 필요(env `KRX_ID`/`KRX_PW`), 세션 수명 약 1시간.
- **(c) 라이선스**: 🚫 **KRX 공지(2026-06-01)가 pykrx 등 사설 라이브러리 (상용) 이용 금지** → 개발/연습 한정. 분류 자체는 KRX 표준 업종(상표 이슈 적음).
- **(d) 체계**: KRX 표준 업종 **대분류(24~26)**. flat, 하위 세부업종 없음(Naver WICS 79보다 거침).
- **(e) 순수 코드 수집**: ⚠️ **원시 curl은 여전히 `LOGOUT`(400) 차단**(§0-2 대조군). 단 **pykrx의 KRX 로그인 세션을 거치면 순수 파이썬(헤드리스 불필요)으로 수집 가능**. 견고성은 라이브러리+로그인 방식 변경에 의존.
- **(f) 식별자**: 6자리 종목코드(바로 조인). ISIN은 별도 매핑 필요.
- **메모**: 공식성이 필요하면 이 경로, 세분도가 필요하면 Naver(2-B), 라이선스 완전 클린이 필요하면 data.go.kr(2-D). curl-only 무인 파이프라인만 목표면 여전히 Naver/data.go.kr 우회가 단순.
...
- **공식 KRX 업종이 필요하면: pykrx `get_market_sector_classifications`**(§2-E, 2026-07-08 신규 확인). KRX 계정 로그인으로 순수 코드 수집(헤드리스 불필요), 시장 전체·기준일 지원. 단 **업종 대분류(24~26)로 Naver보다 거칠고**, **KRX 공지상 상용 금지 → 개발/연습 한정**.
