...
보유 자산의 **현재 상태**를 여러 기준으로 집계해 보여준다. 지원 축과 각 축이 요구하는 데이터는 다음과 같다. (기간 기준 집계는 그레인이 달라 별도 원장에서 다룬다 — §1.5)
...
**§1.2~1.4의 축과 저장 규칙은 보유 스냅샷 원장에 적용된다.** 축이 늘어나는 원장은 이것뿐이므로 축 조합 구조가 필요한 곳도 여기뿐이다. 나머지 두 원장은 축 조합이 고정이라 그레인 정의로 끝난다.
