...
요구사항을 늘어놓으면 하나의 문장으로 압축된다 — **같은 자산을 다른 축으로 반복해서 보여달라.**
...
| 계좌 유형(일반/연금) 구분 | 계좌유형 |
| 현지 통화와 원화 병기 | 통화 |
| 레버리지 비중 *(원천 확보 시)* | 레버리지 |
| ETF를 개별주 기준으로 환산 | (축이 아니라 **렌즈**) |

축이 이미 6개고, 그중 하나는 "원천이 확보되면"이라는 조건부다. **축은 앞으로 더 늘어난다는 뜻이다.**

화면을 단위로 만들면 이 요구가 곱셈으로 번진다. 뷰 6개 × 렌즈 2 × 통화 2 = 24가지 상태를 각각 구현하게 되고, 더 나쁜 것은 **화면마다 손익·비중 계산이 다시 구현된다**는 점이다. 그러면 "종목별 화면의 합계"와 "계좌별 화면의 합계"가 미묘하게 어긋나는 사고가 구조적으로 발생한다. 자산 관리 도구에서 이 종류의 불일치는 기능 결함이 아니라 신뢰의 문제다.

### 1.3 핵심 설계 — 하나의 팩트, 여러 축

저장하는 행은 **한 종류뿐**이다. 계좌 하나에 담긴 종목 하나의 상태.

```
position_line  ── 그레인: (as_of, 계좌, 종목)          2026-07-27 발췌

  계좌         종목                  수량   매입금액      평가금액
  ───────────────────────────────────────────────────────────────
  KIS 위탁     삼성전자                 7     449,400      498,400
  KIS IRP      삼성전자                 3     206,700      213,600
  KIS 위탁     TIGER 미국나스닥100      5     492,000      512,000
  삼성증권      AAPL                    3     868,000      848,000
  KIS 위탁     KRW 예수금               -   3,700,000    3,700,000
```

이 행 집합 하나에서 `GROUP BY`만 바꾸면 서로 다른 뷰가 나온다.

**종목별** — `GROUP BY 종목`

| 종목 | 수량 | 매입금액 | 평가금액 | 평가손익률 |
|---|---:|---:|---:|---:|
| 삼성전자 | 10 | 656,100 | 712,000 | +8.5% |
| TIGER 미국나스닥100 | 5 | 492,000 | 512,000 | +4.1% |
| AAPL | 3 | 868,000 | 848,000 | −2.3% |
| KRW 예수금 | — | 3,700,000 | 3,700,000 | — |

**계좌별** — `GROUP BY 계좌`

| 계좌 | 매입금액 | 평가금액 |
|---|---:|---:|
| KIS 위탁 | 4,641,400 | 4,710,400 |
| KIS IRP | 206,700 | 213,600 |
| 삼성증권 | 868,000 | 848,000 |
...
같은 5줄이 세 개의 질문에 답했고, **집계 로직은 한 벌뿐**이다. 뷰를 늘리는 일이 `group_by`에 컬럼 이름 하나를 더 쓰는 일이 된다. 조건부 요구사항인 레버리지 비중도 `is_leveraged` 축이 생기는 순간 코드 수정 없이 켜진다.

여기서 **렌즈가 왜 축이 아닌지**도 드러난다. 위 시장 비중은 `TIGER 미국나스닥100`을 국내로 집계한다 — 국내 상장 ETF이므로 맞는 답이지만, 사용자가 알고 싶은 "실제로 미국에 얼마나 노출됐나"의 답은 아니다. 렌즈를 켜면 그 512,000이 미국 구성종목으로 풀리면서 **국내 76.4% / 미국 23.6%**로 답이 뒤집힌다. 축을 바꾼 게 아니라 **행 자체를 다시 쓴 것**이다.

### 1.4 이 구조를 떠받치는 세 규칙

구조가 성립하려면 세 가지가 지켜져야 한다. 셋 다 "나중에 조심하자"가 아니라 스키마와 검증으로 강제한다.

**① 가산 가능한 값만 저장한다.** 비율은 더할 수도 평균낼 수도 없다. 위 종목별 표의 수익률을 그냥 평균 내면 이렇게 된다.

```
라인별 수익률 평균 = (8.5 + 4.1 − 2.3) / 3               = +3.4%   ← 틀림
집계 후 계산       = (2,072,000 − 2,016,100) / 2,016,100  = +2.8%   ← 맞음
                      평가 합계     매입 합계
```

그래서 `position_line`에 **평가손익·수익률·비중 컬럼을 아예 만들지 않는다.** 스키마에 자리가 없으면 잘못 더할 방법도 없다. 어떤 축 조합으로 잘라도 숫자가 자동으로 맞는다. (규칙 전문 §3.2)

**② 렌즈는 스키마를 보존하는 변환이다.** ETF 분해를 뷰마다의 분기 옵션으로 두면 모든 뷰 로직이 두 벌이 된다. 대신 **입력도 라인 집합, 출력도 라인 집합**인 함수 하나로 둔다. 출력 형태가 입력과 같으므로 그 아래 집계·비중·환산 로직은 손대지 않는다. 렌즈는 파이프라인 앞단에서 한 번 갈아끼우는 부품이 되고, UI 토글이 파라미터 한 개로 그대로 내려간다. (§3.4)

**③ 원장을 3개로 분리한다.** 확보 난이도가 전혀 다른 데이터를 한 테이블에 섞지 않는다.

| 원장 | 확보 | 답하는 것 |
|---|---|---|
| 보유 스냅샷 | **항상 가능** — 잔고가 진실 | 현재 상태 전부 |
| 현금흐름 | 대체로 가능 | 자산 변화의 원인 |
| 거래 | **구간별 불완전** | 실현/미실현 분해 |

이 분리의 값어치는 결과에서 드러난다. **6개 뷰 중 4개가 "항상 신뢰 가능한 원장"만 사용한다.** 이 프로젝트 최대 약점인 거래내역 불완전이 제품 전체로 번지지 않고 실현손익 뷰 한 곳에 갇힌다. 나아가 자산 변화 뷰는 항등식을 뒤집어 **거래 원장 없이도 기간 손익을 계산**하므로, 체결내역 조회가 막힌 연금계좌까지 정확한 답을 얻는다. (§4)

### 1.5 검토한 대안

| 안 | 내용 | 판단 |
|---|---|---|
| **A. 팩트 1개 + 축 조합** | 위 설계 | **채택** |
| B. 뷰별 전용 집계 테이블 | 뷰마다 mart 테이블을 만들고 백엔드는 조회만 | 조회는 빠르지만 **축 추가 = 테이블·파이프라인 추가**이고 렌즈 때문에 테이블이 두 배가 된다. 무엇보다 뷰마다 손익·비중을 재구현해 §1.2의 합계 불일치가 그대로 발생한다 |
| C. 거래 원장에서 전 상태 재구성 | 이벤트 소싱. 모든 상태를 체결 이력에서 계산 | 정확도는 이상적이나 **프로젝트 대전제와 정면 충돌**한다. 거래내역은 구간별로 불완전하고 잔고 스냅샷이 진실이며, 기업행위 이력이 없으면 평단이 부호까지 틀어진다 |

B는 A의 조회 성능 이점만 따로 취할 수 있다 — A로 모델링하고 EOD 배치 결과를 집계 캐시로 머티리얼라이즈하면 된다. 구조를 B로 잡을 이유는 없다.
...
| 비용 | 대응 |
|---|---|
| 조회할 때마다 조인 + 집계가 필요하다 | EOD 배치가 만든 집계 캐시로 상쇄한다(§3.3) |
| 뷰가 사양 데이터라 "이 숫자 어디서 나왔나" 추적이 한 단계 간접적이다 | UI ↔ 모델 매핑표를 스펙에 고정한다(§8) |
| **팩트 그레인을 잘못 잡으면 전 화면이 함께 틀어진다** | 그레인 유일성을 저장 전 검증 관문으로 둔다(§9.1) |

세 번째가 이 설계의 진짜 위험이다. 단일 실패점을 만드는 대신 그 지점을 좁고 명시적으로 만들었다 — 검증할 곳이 하나뿐이라는 뜻이기도 하다.
