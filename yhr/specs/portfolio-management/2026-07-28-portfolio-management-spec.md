# 포트폴리오 종합 관리 (Portfolio Management) — 설계 스펙

- **버전**: v1 (2026-07-28)
- **상태**: 설계 합의 완료 → 구현 계획 착수 전
- **범위**: 뷰 정의 및 화면 UX, 집계 모델, 데이터 모델(DB 저장 구조), 3원장과 신뢰도 등급, 계좌 연동·동기화 흐름과 상태, 인증과 사용자 스코프, 검증 규칙
- **비범위(이번 스펙에서 다루지 않음)**: 데이터 수집 구현 상세(KIS TR·CODEF 엔드포인트별 호출 방식), 인프라·파이프라인 구현, 알림 발송, PER/PBR 섹터 분위수, 벤치마크 비교
- **관련 자료**: [`wireflow.drawio`](./wireflow.drawio) / [`wireflow.png`](./wireflow.png) / [`wireflow_src.py`](./wireflow_src.py)(두 산출물 생성기) · [1차 기능 명세·기술 검토](../2026-06-28-feature-spec-and-tech-review.md) · [매수·매도 타이밍 알림 스펙](../buy-sell-timing-alert/2026-07-12-buy-sell-timing-alert-spec.md) · [팀 아키텍처 합의](../../../team-architecture-agreement.md)

---

## 1. 개요

### 1.1 기능 요약

여러 증권사·기관에 흩어진 보유 자산을 하나로 모아, **종목·섹터·시장·통화·계좌** 축으로 비중과 손익을 보여준다. 보유 ETF는 구성종목으로 분해(look-through)해 실제 익스포저를 계산한다.

평가금액·손익은 **EOD 일배치**로 계산한다(실시간 아님). 사용자는 필요할 때 수동 새로고침으로 온디맨드 동기화를 트리거한다.

사용자마다 본인 키로 본인 계좌를 연동하고 **본인 자산만 본다.** 소유권 축은 계좌에 둔다 — `account.user_id`가 유일한 소유 표시이고, 보유 스냅샷·실현손익·입출금은 계좌를 통해 소유자가 결정된다(§3.8 · §5.1). 데이터 범위가 사용자마다 갈리므로 **인증이 필수**다(§8.8).

### 1.2 축

보유 자산의 **현재 상태**를 여러 기준으로 집계해 보여준다. 지원 축과 각 축이 요구하는 데이터는 다음과 같다. (기간 기준 집계는 그레인이 달라 별도 원장에서 다룬다 — §1.4)

| 축 | 값 | 필요 데이터 | 원천 |
|---|---|---|---|
| 계좌 | 계좌 단위 | 계좌 마스터 | KIS · CODEF |
| 계좌유형 | 일반 / 연금 | 계좌 마스터 | KIS · CODEF |
| 종목 | 종목 단위 | 종목 마스터(ISIN) | 데이터팀 종목 마스터 |
| 섹터 | 반도체 · 소프트웨어 · … | 종목 → 섹터 분류 | yfinance(미국) · Naver 업종(국내) |
| 시장 | 국내 / 미국 | 종목 마스터 메타 | 종목 마스터 |
| 통화 | KRW / USD | 종목 마스터 메타 + 환율 | 종목 마스터 · python-kis `exchange_rate` |
| 자산군 | 주식 / ETF / 현금 | 종목 마스터 메타 | 종목 마스터 |
| 레버리지 | 레버리지 여부 | 종목 속성 | 미확보 — 확보 시 활성화 |

축과 별개로 **렌즈**가 하나 있다. 축이 집계 기준을 바꾸는 반면, 렌즈는 집계 대상 행 자체를 바꾼다.

| 렌즈 | 동작 | 필요 데이터 | 원천 |
|---|---|---|---|
| `DIRECT` | ETF를 한 종목으로 집계 | — | — |
| `LOOK_THROUGH` | ETF를 구성종목으로 분해 | ETF 구성종목 · 비중 | SPDR XLSX(미국) · 국내 원천 확보 전제 |

### 1.3 그레인

축은 8개지만 저장 단위는 하나다. 모든 축은 (계좌 × 종목)보다 거칠고, 증권사 잔고도 이 단위로 들어온다.

```
position_line  ── 그레인: (as_of, 계좌, 종목)
```

한 행이 계좌 하나와 종목 하나를 가리키므로, 두 마스터를 조인하면 **§1.2의 축 8개가 모두 결정된다.**

| 축 | 유도 |
|---|---|
| 계좌 · 계좌유형 | `account_id` → 계좌 마스터 |
| 종목 · 섹터 · 시장 · 통화 · 자산군 · 레버리지 | `instrument_id` → 종목 마스터 |

축은 이렇게 행에 딸린 속성이고, 뷰는 그 위의 `GROUP BY`다.

| 뷰 | group_by |
|---|---|
| 요약 | (없음) |
| 종목별 | 종목 |
| 비중 분석 | 섹터 / 시장 / 통화 / 자산군 / 종목 / 레버리지(조건부) |
| 계좌별 | 계좌유형 → 계좌 |

축을 늘리는 일은 `group_by`에 컬럼 이름을 하나 더 쓰는 일이다. 섹터·자산군 같은 분류 축은 라인에 복사하지 않고 마스터에서 조인한다. 분류를 보정하면 과거 스냅샷에도 소급 반영된다.

### 1.4 원장

그레인이 다른 데이터는 같은 팩트에 담기지 않는다. 실현손익은 체결 1건 단위, 자산 변화는 기간 단위여서 `position_line`에 들어갈 수 없다. 원장을 셋으로 나눈다.

| 원장 | 그레인 | 축 | 답하는 것 | 확보 |
|---|---|---|---|---|
| 보유 스냅샷 `position_line` | as_of × 계좌 × 종목 | §1.2의 8개, **확장됨** | 현재 상태 전부 | 항상 — 잔고가 진실 |
| 현금흐름 `cln_cashflow` | 계좌 × 유형 × 일시 | 기간 × 유형, 고정 | 자산 변화의 원인 | 입출금내역 |
| 거래 `cln_trade` | 체결 1건 | 기간 × 종목, 고정 | 실현/미실현 분해 | 구간별 불완전 |

축이 늘어나는 원장은 보유 스냅샷뿐이다. **앞의 축(§1.2~1.3)과 뒤의 저장 규칙(§1.5)은 이 원장에 적용된다.** 나머지 두 원장은 축 조합이 고정이라 그레인 정의로 끝난다.

확보 난이도가 서로 다르므로 분리는 리스크 격리도 겸한다. 6개 뷰 중 4개(요약 · 종목별 · 비중 분석 · 계좌별)는 보유 스냅샷만 사용한다. 실현손익만 거래 원장에 의존해 신뢰도 등급이 붙고(§4.4), 자산 변화는 스냅샷과 현금흐름으로 기간 손익을 구하므로 거래 원장 없이 성립한다(§2.9).

### 1.5 저장 규칙

**금액과 수량만 저장한다.** 비율은 가산이 불가능해 라인에 두면 축이 바뀔 때 값이 틀어진다. 평가손익·수익률·비중 컬럼은 스키마에 만들지 않고 집계 후에 계산한다.

| 저장 (가산 O) | 파생 (집계 후) |
|---|---|
| `quantity` · `cost_amount_*` · `market_value_*` | 평가손익 = Σ평가 − Σ매입 |
| | 평가손익률 = 평가손익 ÷ Σ매입 |
| | 비중 = Σ평가 ÷ 전체 Σ평가 |

**원화 환산값은 라인에 기록한다.** `market_value_krw`를 `fx_rate` · `fx_as_of`와 함께 저장한다. 집계 시점에 환산하면 통화가 섞인 그룹에서 적용 환율이 모호해진다.

**렌즈는 라인 집합을 라인 집합으로 바꾼다.** 입출력 스키마가 같으므로 하위의 집계·비중·환산 로직은 렌즈 적용 여부와 무관하다.

```
DIRECT        [TIGER 미국나스닥100  512,000]
                    │
LOOK_THROUGH  [AAPL 148,000] [MSFT 121,000] … [기타 16,000]     Σ 보존
```

### 1.6 용어 (Glossary)

| 용어 | 의미 |
|------|------|
| **그레인 (grain)** | 팩트 테이블의 한 행이 무엇을 나타내는가. 차원 모델링(dimensional modeling)의 표준 용어. 집계가 중복·누락 없이 한 번씩 일어나게 하는 기준 |
| **팩트 (fact)** | 측정값을 담는 행 집합. `position_line`이 스냅샷 원장의 팩트 |
| **축 (axis)** | 집계의 그룹핑 기준. 계좌·계좌유형·종목·섹터·시장·통화·자산군·레버리지(조건부) |
| **스코프 (scope)** | 집계 대상 행이 애초에 누구 것인가. 축·필터와 달리 사용자가 고를 수 없고 인증 주체가 정한다 (§3.8) |
| **측정값 (measure)** | 더해도 되는 값. 수량·매입금액·평가금액 |
| **파생 지표 (derived)** | 더할 수 없는 값. 평가손익률·비중. 집계 **후**에만 계산 |
| **렌즈 (lens)** | 라인 집합을 라인 집합으로 바꾸는 변환. `DIRECT` / `LOOK_THROUGH` |
| **원장 (ledger)** | 서로 다른 진실의 출처. 보유 스냅샷 / 현금흐름 / 거래 |
| **신뢰도 등급 (grade)** | 거래 원장의 완전성 판정. `VERIFIED / SEEDED / UNAVAILABLE / CONFLICT`. 실현손익 응답의 종목 노드는 `MIXED`가 추가된다 |
| **as-of** | 그 행이 나타내는 시점. 스냅샷의 기준일자이자 `position_line` 그레인의 일부. 적재 시각, 계좌별 실제 데이터 시각(`source_as_of`)과 구분 |
| **캐리포워드** | 그날 `collection_run` 최종 상태가 `DONE`이 아닐 때 직전 성공 스냅샷을 그날 기준일자로 이월 |

---

## 2. 뷰 모델

### 2.1 뷰 목록

뷰의 정체성은 **답하는 질문**과 **그레인** 두 가지로 정의된다.

| 뷰 | 답하는 질문 | 그레인 | 원장 | 렌즈 |
|---|---|---|---|---|
| **요약** | 지금 내 자산 상태는? | 포트폴리오 전체 1행 | 스냅샷 | 미니차트 한정 |
| **종목별** | 뭘 얼마나 갖고, 각각 얼마 벌었나? | 종목 1행(계좌 합산) | 스냅샷 | 선택 |
| **비중 분석** | 내 돈이 어디에 쏠려 있나? | 축 값 1행 | 스냅샷 | **전면** |
| **계좌별** | 어느 기관·계좌유형에 뭐가 있나? | 계좌 1행 | 스냅샷 | 없음 |
| **실현손익** | 이번 달/올해 확정한 손익은? | 기간 × 종목 × 매도 체결 | 거래 | 없음 |
| **자산 변화** | 는 게 벌어서인가, 넣어서인가? | 기간 × 현금흐름 유형 | 스냅샷 + 현금흐름 | 없음 |

진입 화면: **로그인**. 하위 화면: **종목 상세**, **계좌 연동·동기화**.

앞의 4개 뷰는 같은 팩트를 그룹핑만 바꾼 것이다(§1.3). 실현손익과 자산 변화만 다른 원장을 쓴다.

### 2.2 네비게이션

하단 탭바는 **5개**다 — 요약 · 종목 · 비중 · 계좌 · 손익(실현손익). **자산 변화는 탭이 아니다.**

탭 6개는 하단바에 과밀하고, 무엇보다 자산 변화는 **질문이 생긴 순간에 도달해야 의미가 있는 화면**이다. 진입 경로를 두 곳의 맥락 링크로 고정한다.

| 진입점 | 링크 |
|---|---|
| 요약 — 타일 아래 | `이번 달 자산 +1,200,000원   더보기 →` |
| 실현손익 — 목록 하단 | `이 기간 전체 투자손익 보기 →` |

종목 상세와 계좌 연동도 탭이 아니라 하위 화면이다(각각 종목별 행 탭, 계좌별 화면에서 진입).

로그인은 탭바 바깥의 진입 화면이다. 인증되지 않은 상태에서는 어느 탭도 그릴 수 없어 앱 진입 시 먼저 도달하고, 토큰이 만료되면 어느 화면에서든 여기로 돌아온다(§8.8).

### 2.3 공통 전역 요소

모든 뷰 상단에 고정된다.

- **as-of 배너**: `기준 2026-07-27 15:30 · 새로고침`. 배치 실행 기준일자를 표기하고, 새로고침은 **그 화면의 뷰를 다시 조회한다.** 필터·축·렌즈 상태는 유지된다. 수동 동기화(§7.6)는 계좌 단위 행위이므로 계좌별 화면에서 트리거한다. 자산 변화는 기간 구간을 기간 컨트롤 아래에 함께 표기한다.
- **환율 표기**: 원화 환산에 적용한 환율과 기준시점을 배너에 병기한다. 렌즈를 켠 화면은 구성비중 기준일도 함께 병기한다(§3.4). 통화 표시 규칙은 §3.7.
- **품질 경고**: `notices`를 `severity`로 나눠 배치한다. `info`는 배너 둘째 줄에 이어 붙이고, `warn`·`error`는 배너 아래 줄로 쌓는다. 문구는 서버 `message`를 그대로 출력하고 `code`로 아이콘·색·후속 동작만 분기한다(§8.2).

계좌 단위 상태(캐리포워드·재인증)는 전역 배너가 아니라 **계좌별 화면**에 둔다. 그 화면이 계좌마다 다른 사실을 행으로 말할 수 있고 조치 경로도 거기 있기 때문이다. 다른 화면에서는 하단 탭바의 `계좌` 아이콘에 점을 띄워 알린다(§7.5).

### 2.4 요약

| | |
|---|---|
| 그레인 | 포트폴리오 전체 1행 |
| 지표 | 총자산(예수금 포함) · 유가증권 평가금액 · 총매입 · 총평가손익(금액/%) · 현금비중 · 전일 대비 일간 변화 · 계좌수·종목수 |
| CASH 범위 | 총자산·비중·현금비중은 예수금 포함, 손익 계열은 제외(§6.2) |
| 보조 | 자산구성 미니차트(시장 축 고정) |
| 렌즈 | 미니차트에만 적용. 화면에 토글을 노출하지 않고 `DIRECT`로 고정 호출한다 |

**화면 구성**

```
총자산            58,000,000원
                 +4,500,000 (+9.2%)        ← 총평가손익

┌──────────────┬──────────────┐
│ 총매입금액    │ 직전 거래일 대비│
│ 48,800,000   │ +1,200,000    │
├──────────────┼──────────────┤
│ 현금비중      │ 계좌 · 종목    │
│ 8.1%         │ 4계좌 · 6종목  │
└──────────────┴──────────────┘

이번 달 자산 +1,200,000원          더보기 →

자산 구성                            비중 →
▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓░░░░░░
● 국내  46,800,000원          80.7%
● 미국  11,200,000원 ($8,000) 19.3%
```

**유가증권 평가금액은 라벨로 노출하지 않는다.** 총매입금액과 총평가손익에서 유도되고, 단독으로 궁금해지는 순간이 없다. 회계 용어를 그대로 화면 라벨로 쓰면 읽히지 않는다.

일간 변화는 **직전 `as_of` 스냅샷과의 차이**로 정의된다. `as_of`가 영업일에만 생성되므로 월요일은 금요일과 비교한다. 장중 실시간 등락이 아니며, 라벨에 `직전 거래일 대비`를 명시한다. 직전 스냅샷이 없는 첫날은 `0`이 아니라 `null`이며 화면에 `—`로 표시한다.

자산 변화 링크는 **금액을 함께 보여준다.** 그 값은 `asset-change` 응답의 `closing − opening`이므로 요약 화면은 `summary`와 `asset-change?period=THIS_MONTH` 두 응답을 함께 받아 한 번에 그린다. 기초 스냅샷이 없어 실제 시작일이 당겨진 경우(§2.9)에도 맞는 문구여야 하므로 `전월 대비`가 아니라 `이번 달`로 쓴다. 총자산 변화는 입출금을 포함하므로 수익과 같지 않다.

**미니차트는 시장 축 고정이고 형식은 100% 누적 막대다.** 축 전환과 렌즈는 비중 분석 뷰의 일이며 요약에 두면 두 화면의 역할이 겹친다. 자산군 축은 바로 위 현금비중과 정보가 겹치는 반면 시장 축은 겹치지 않고, 그룹이 단일 통화라 현지 통화 병기가 성립한다(§3.7). 조각이 둘뿐이라 도넛은 세로 공간만 쓴다. 예수금은 시장 축에서 해당 시장 안에 포함되므로 막대에 별도 현금 조각이 생기지 않는다.

### 2.5 종목별

| | |
|---|---|
| 그레인 | **종목 1행 (모든 계좌 합산)** |
| 행 구성 | 1줄 `종목명 · 시장 배지 · 평가금액 · 비중` / 2줄 `수량 · 평단 · 평가손익(금액/%)` |
| 통화 | 외화 종목은 현지 통화와 원화를 병기(§3.7). 합계·비중은 원화 |
| 필터 | 계좌 / 시장 / 자산군. 모두 다중 선택이며 선택지는 카탈로그가 내린다(§6.4) |
| 정렬 | 서버는 평가금액 내림차순 고정. 화면 정렬(평가금액·평가손익·손익률·이름)은 클라이언트가 한다 |
| 렌즈 | 선택 — "구성종목 기준으로 보기" |
| 하위 | 행 탭 → 종목 상세 |

같은 종목을 여러 계좌에 보유하면 **항상 합산해 1행으로 표시**하고, 평단은 수량가중평균으로 계산한다. 계좌별 분해는 종목 상세에서 제공한다.

```
삼성전자 [국내]        14,240,000원 (24.6%)
200주 · 평단 60,000    +2,240,000 (+18.7%)
```

**행은 표가 아니라 2줄 카드다.** 컬럼을 늘어놓으면 모바일 폭에서 셋만 남고 나머지를 어딘가로 미뤄야 하지만, 2줄이면 종목명·배지·평가금액·비중·수량·평단·평가손익이 한 카드에 다 들어간다. 종목명은 최대 1줄이고 폭이 모자라면 뒤쪽을 말줄임한다 — 금액은 어떤 경우에도 자르지 않는다.

**현재가와 매입금액은 행에 두지 않는다.** 현재가는 지표 카탈로그에 없고(§6.2), 매입금액은 `평가금액 − 평가손익`으로 즉시 유도된다.

**예수금 의사종목도 행으로 남긴다.** 빼면 비중의 합이 100%가 되지 않고 평가금액 합계가 `total_assets_krw`와 어긋난다. 원가·손익 자리는 `—`이며, 목록에서 제외하려면 자산군 필터를 쓴다.

합계 줄은 필터를 반영한 `total`을 쓴다. 여기에 종목수를 병기하지 않는다 — `instrument_count`는 예수금을 세지 않아 목록의 행 수와 어긋난다.

렌즈를 켜면 **2줄이 환산수량만 남는다.** 평단·매입금액·평가손익이 행에서 빠지고(§3.4) 시장·자산군 필터가 비활성되며(§9.3), 그 사유는 `LENS_METRICS_OMITTED` 문구를 토글 옆 도움말로 보여준다.

### 2.6 비중 분석

| | |
|---|---|
| 그레인 | 축 값 1행 |
| 축 | 종목 · 섹터 · 시장(국내/미국) · 통화 · 자산군(주식/ETF/현금) · 레버리지(원천 확보 시) |
| 지표 | 평가금액(원화) · 비중% · 종목수 |
| 필터 | 계좌 · 계좌유형 |
| 렌즈 | **핵심 용처.** 기본 `DIRECT`, 토글로 `LOOK_THROUGH` |
| 시각화 | 순위 막대 리스트 |

축 전환은 세그먼트로 한다. 전환이 이 화면의 주 행위라 한 번에 닿아야 하고, 접힌 목록은 무엇으로 볼 수 있는지를 감춘다. 세그먼트에 그리는 축은 카탈로그의 `enabled`가 참인 것뿐이며, 레버리지 축은 원천을 확보하면 서버가 값을 바꾸는 것만으로 나타난다(§6.1 · §6.4).

**손익은 행에 두지 않는다.** 이 화면의 질문은 쏠림이고 손익은 종목별 뷰가 답한다. 렌즈를 켜면 손익이 행에서 빠지므로(§6.2) 기본 상태에 두면 토글할 때마다 행 모양이 바뀐다.

**도넛을 두지 않는다.** 같은 값을 순위 막대와 두 번 그리게 되고, 종목 축에서는 조각이 수십 개로 늘어 색으로 구분되지 않는다. 길이 비교가 각도 비교보다 정확하고 행이 늘어도 무너지지 않는다.

렌즈를 켜면 **`기타(ETF 내 비주식·미매칭)` 버킷이 함께 표시**되어 합이 항상 100%가 되게 한다. 예수금은 `현금` 값으로 별도 집계되므로 라벨을 구분해 두 종류의 현금이 한 화면에서 겹치지 않게 한다. ETF가 보유한 현금·선물·채권, weight 합의 반올림 오차, 종목 마스터에 매칭되지 않는 구성종목이 여기 들어간다. 이는 데이터 확보 실패가 아니라 **원래 종목이 아닌 것**이므로 별도 버킷으로 유지한다.

기타 버킷의 라벨은 행에 그대로 쓰기에 길다. 화면은 `key`가 `OTHER`인 행만 `기타`로 줄여 쓰고 전문은 도움말로 보여준다. 라벨 문자열이 아니라 키로 분기하므로 문구를 고쳐도 화면이 깨지지 않는다.

섹터 축에서 ETF는 섹터가 비어 `미분류`로 모인다. 그 행에 도움말을 붙여 렌즈를 켜면 실제 섹터가 나타난다고 안내한다(§6.1).

렌즈를 켜면 배너에 구성비중 파일의 기준일을 병기한다 (§3.4).

### 2.7 계좌별

| | |
|---|---|
| 그레인 | 계좌 1행 |
| 행 구성 | 1줄 `기관 · 상태 배지 · 계좌 총자산 · 비중` / 2줄 `예수금 · 평가손익(금액/%)` |
| 응답 키 | `market_value_krw`(계좌 총자산) · `deposit_krw` · `link_state` · `source_as_of` · `is_carried_forward` |
| 그룹 | **계좌유형별 소계** (일반 합계 / 연금 합계). 소계는 예수금을 포함한 계좌 총자산 기준이며 그룹 헤더 오른쪽에 둔다 |
| 렌즈 | 없음 |
| 하위 | 행 탭 → 종목별 뷰(계좌 필터 적용 상태) |
| 겸용 | 계좌 연동·재인증·해제의 진입점 |

계좌유형 그룹은 기본으로 펼쳐 두고 헤더 탭으로 접을 수 있다. 계좌가 늘면 접어서 훑는 편이 낫고, 접힌 그룹 안의 상태 배지는 전역 경고 줄이 대신 알린다.

**계좌유형은 행에 두지 않는다** — 그룹이 이미 말한다. **유가증권 평가금액도 두지 않는다** — 계좌 총자산과 예수금이 나란히 있으면 유도된다.

**계좌 단위 상태는 행이 말한다.** 캐리포워드된 계좌는 `source_as_of`를 `07-24 기준` 배지로, 재인증·연동 이상은 `link_state`를 배지로 붙인다. 전역 경고 줄(`STALE_ACCOUNTS` · `REAUTH_REQUIRED`)은 목록이 길어져 배지가 화면 밖으로 나갔을 때를 위해 함께 둔다.

마지막 동기화 시각은 동기화 기능이 생기기 전까지 표시하지 않는다. 값이 없는 자리를 만들면 화면이 비어 보이고, 지어낸 값을 넣으면 화면이 거짓말을 한다.

렌즈는 계좌 합계를 바꾸지 못한다(look-through는 총합을 보존하므로). 따라서 이 뷰에는 렌즈를 노출하지 않는다.

### 2.8 실현손익

| | |
|---|---|
| 그레인 | **기간 × 종목 × 매도 체결** |
| 원장 | 거래 원장 전용 |
| 기간 | 이번 달 · 지난달 · 올해 · 작년 · 지정 |
| 행 구성 | 1줄 `종목 · 신뢰도 배지 · 실현손익` / 2줄 `최근 매도일 · 매도금액 · 실현손익률` |
| 펼침 | 종목 행을 탭하면 매도 체결이 펼쳐진다. 체결마다 매도일 · 수량 · 매도금액 · 취득원가 · 실현손익 · 등급 배지 |
| 필터 | 계좌 — 전체 / 계좌유형(일반·연금) / 개별 계좌. 계좌유형은 그 유형의 계좌 ID로 펼쳐 요청한다 |
| 통화 | 종목 행은 병기, 기간 합계는 원화(§3.7) |
| 렌즈 | 없음 (ETF는 ETF 단위로 체결됨) |

**미실현 평가손익은 이 뷰에 넣지 않는다.** 요약(총평가손익)과 종목별(종목당 평가손익)에서 이미 제공되며, 기간 확정값과 시점 평가값을 한 화면에서 더하면 어떤 질문에도 답하지 않는 숫자가 나온다.

실현손익만 보면 이익 난 종목만 매도한 착시가 생기므로, 하단에 `이 기간 전체 투자손익 보기 →` 링크로 자산 변화 뷰를 연결한다.

합계는 확보된 계좌만 합산하고 `미포함 계좌 N개`를 함께 표시한다. 조용히 제외하지 않는다. 이 문구와 `N개 종목은 추정치입니다`는 합계의 성질을 수식하므로 화면 상단 경고 줄이 아니라 합계 바로 아래에 둔다.

취득원가는 펼침에만 둔다. `실현손익 = 매도금액 − 취득원가 − 비용`이라 뺄셈으로 유도되지는 않지만, 종목 행이 답하는 것은 얼마 벌었는가이고 원가는 그 근거다.

**체결을 종목으로 접지 않는다.** 등급은 체결의 속성이라 한 종목 안에서도 체결마다 갈린다(§4.5). 종목으로 접으면 그 정보가 사라지고 "섞이면 낮은 쪽" 같은 임의 규칙이 생긴다. 대신 §8.3의 2단계 중첩(`rows[].rows`)을 그대로 써서 화면에서 종목별로 묶는다. 종목 노드의 `grade`는 체결 등급이 하나면 그 값, 섞이면 `MIXED`이며 이는 접기 결과가 아니라 펼쳐 보라는 신호다.

등급 배지를 탭하면 사유를 도움말로 보여준다. `position_basis`가 없는 동안은 확보 구간 시작일과 시드 출처를 알 수 없으므로 두 원인을 함께 말하는 고정 문구를 쓰고, 원인을 지어내지 않는다.

연금계좌는 과세이연이라 일반계좌와 나눠 보는 수요가 크므로 계좌 필터를 제공한다.

### 2.9 자산 변화

| | |
|---|---|
| 그레인 | 기간 × 현금흐름 유형 |
| 원장 | 보유 스냅샷 + 현금흐름 |
| 기간 | 이번 달 · 지난달 · 올해 · 작년 · 지정 |
| 필터 | 계좌 — 전체 / 계좌유형(일반·연금) / 개별 계좌 |
| 구성 | 기초자산 → 넣은 돈 → 번 돈 → 계좌 편입·제외 → 기말자산 |
| 렌즈 | 없음 |

**항등식**

```
기말 총자산 = 기초 총자산
            + 넣은 돈          입금 - 출금
            + 번 돈            투자손익 + 배당 - 수수료·세금
            ± 계좌 편입·제외
```

투자손익은 **나머지 전부**로 정의한다.

```
투자손익 = Δ총자산 - (입금 - 출금) - 배당 + 수수료·세금 - 계좌 편입·제외
```

우변에 거래 원장이 들어가지 않는다. 스냅샷 두 개(기초·기말)와 현금흐름만 있으면 기간 손익이 나오므로, **체결내역 조회가 차단된 계좌(연금 등)도 정확한 손익을 얻는다.** 거래 원장은 그 손익을 실현/미실현으로 쪼갤 때만 필요하다. 이 뷰가 거래내역 불완전에 대한 백스톱 역할을 한다.

**투자손익의 범위** — 기간 중 매도해 확정된 손익과, 기말까지 보유한 자산의 평가 변동을 합한 값이다. 후자는 미실현손익 자체가 아니라 그 **변화분**(기말 미실현 - 기초 미실현)이다. 따라서 실현손익 뷰의 숫자와 일치하지 않는다.

**"넣은 돈"은 사용자 입력에서 온다.** 증권사 입출금 이력에 기대지 않으므로(§4.6) 입력이 없는 기간은 그만큼 투자손익에 섞인다. `넣은 돈` 행에서 바로 입력 화면으로 갈 수 있게 한다.

**배당과 수수료는 "번 돈"에 넣는다.** 이 뷰가 답하는 질문이 "는 게 벌어서인가 넣어서인가"이므로, 자산이 낳은 배당과 투자에 든 비용은 시장 쪽에 귀속된다. 사용자가 넣은 돈은 입출금뿐이다.

**화면 구성**

```
┌──────────────────────────────────────────────┐
│ ‹  자산 변화                                  │
│ [이번 달][지난달][올해][작년][지정]            │
│ [전체 계좌 ▾]                                 │
│ 2026-07-01 ~ 2026-07-31 · 07-24부터 계산      │
├──────────────────────────────────────────────┤
│   자산이 120만원 늘었는데                      │
│   200만원을 넣고 80만원을 잃었어요             │
├──────────────────────────────────────────────┤
│   기초자산                        56,800,000  │
│                                               │
│   넣은 돈                         +2,000,000  │
│     입금          ▏▬▬▬▬▬▬▬▬▬▬    +2,000,000  │
│                                               │
│   번 돈                             -800,000  │
│     투자손익  ▬▬▬▬▏                 -800,000  │
│   ─────────────────────────────────────────   │
│   기말자산                        58,000,000  │
├──────────────────────────────────────────────┤
│  투자손익 -800,000원의 내역                    │
│   거래내역이 없어 확정·평가로 나누지 못했어요   │
└──────────────────────────────────────────────┘
```

**막대는 0을 기준으로 좌우로 그린다.** 기초자산에서 출발하는 전통적인 워터폴은 이 화면에서 성립하지 않는다 — 기초자산이 5,680만원인데 항목이 200만·80만이면 항목 막대가 10pt 이하가 되어 보이지 않는다. 비교해야 할 것은 넣은 돈과 번 돈이지 기초자산이 아니므로, 막대 스케일을 항목 최대값에 맞추고 기초·기말은 목록의 처음과 끝에 숫자로 둔다.

기간 구간과 실제 시작일은 컨트롤 바로 아래에 둔다. 사용자가 방금 고른 값이라 컨트롤 옆이 제자리이고, 기초 스냅샷이 없어 시작일이 당겨진 사실도 같은 자리에서 읽힌다.

**UX 원칙**

1. **답을 문장으로 먼저 준다.** 차트를 읽기 전에 질문이 해결되어야 한다.
2. **넣은 돈과 번 돈을 두 덩어리로 가른다.** 경계는 그룹 헤더와 들여쓰기가 지고 **색은 증감만 뜻한다** — 늘어난 항목은 빨강, 줄어든 항목은 파랑. 색에 출처를 싣던 방식은 손실을 초록으로 칠하게 되고, 앱 전체에서 파랑이 하락을 뜻하는 것과도 충돌한다. 계좌 편입·제외는 두 덩어리 어디에도 속하지 않으므로 회색으로 둔다.
3. **투자손익만 강조한다.** 나머지 항목은 근거이고 답은 하나다.
4. **분해는 아래로 미룬다.** 실현/미실현 분해는 거래 원장이 필요한 불확실 영역이므로 확실한 답 아래에 보조로 둔다.

이 뷰가 존재하는 결정적 이유는 **자산이 늘었는데 손실인 상황**이다. 다른 어떤 뷰로도 드러나지 않는다.

```
자산이 200만원 늘었지만
350만원을 넣고 150만원을 잃었어요
```

**경계 처리**

| 상황 | 처리 |
|---|---|
| 기초 스냅샷 없음 | 가장 이른 스냅샷을 기초로 쓰고 `2026-07-15부터 계산`처럼 실제 시작일을 표시 |
| 기간 중 계좌 연동 | `＋ 계좌 편입` 항목. 금액은 그 계좌 첫 스냅샷의 총자산 |
| 기간 중 계좌 해제 | `－ 계좌 제외` 항목. 금액은 마지막 스냅샷의 총자산 |
| 기간 경계에 이월 계좌 | 경고 표시. 그 시점의 참값을 모르므로 오차는 정량화하지 않는다 |
| 입출금 미입력 기간 | 경고 표시(§4.6). 그만큼 투자손익에 섞인다. `넣은 돈` 행에서 입력 화면으로 유도 |
| 값이 0인 항목 | 행을 숨긴다 |
| 계좌 간 이체 | 계좌 필터를 걸면 이체가 양쪽에 입금·출금으로 나타난다. 전체 보기에서는 상쇄된다. 이체 여부를 식별하지 않으므로 계좌별 보기에 `다른 계좌에서 옮긴 돈도 입금으로 표시돼요`를 각주로 둔다 |

계좌 편입·제외는 사용자가 넣은 돈도 시장이 만든 돈도 아니므로 두 덩어리 어디에도 넣지 않고 별도로 표시한다.

### 2.10 종목 상세 (하위 화면)

보유 요약 · **계좌별 분해**(계좌마다 다른 평단) · 해당 종목 거래내역과 실현손익 · 기업행위 이력.

보유 요약과 계좌별 평단은 잔고 기준, 실현손익의 취득원가는 재구성 기준이다. 같은 화면에 놓이지만 원가가 다르므로 실현손익 영역에 그 사실을 표기한다(§4.1).

---

## 3. 집계 모델

### 3.1 팩트 그레인

```
position_line  ── 그레인: (as_of, account_id, instrument_id)
```

모든 스냅샷 기반 뷰가 이 행 집합 하나를 읽는다. 그레인이 고정되어야 집계가 중복·누락 없이 한 번씩 일어난다.

### 3.2 가산성 규칙

비율은 더할 수도 평균낼 수도 없다.

```
삼성전자    매입 1,000만  평가 1,100만  → +10%
SK하이닉스  매입   100만  평가   130만  → +30%

라인별 수익률 평균 = (10 + 30) / 2             = +20%     ← 틀림
집계 후 계산       = (1,230 − 1,100) / 1,100   = +11.8%   ← 맞음
```

| 저장 (가산 O) | 파생 (집계 후) |
|---|---|
| quantity · cost_amount_* · market_value_* | 평가손익 = Σ평가 − Σ매입 |
| | 평가손익률 = 평가손익 ÷ Σ매입 |
| | 비중 = Σ평가 ÷ 전체 Σ평가 |

**파생 지표는 스키마에 컬럼을 만들지 않는다.** 자리가 없으면 잘못 더할 방법도 없다. 규칙을 문서가 아니라 테이블 구조로 강제한다.

### 3.3 축과 필터

축은 결과를 **어떻게 묶을지**를 정하고 그레인을 결정한다. 필터는 **어떤 행을 대상으로 할지**를 정하고 그레인을 바꾸지 않는다. 따라서 축에 없는 값으로도 필터할 수 있으며, 필터 가능 여부는 **마스터 조인 후 라인에 그 값이 붙는가**로 결정된다(§3.6 3.5단계).

실현손익과 자산 변화는 축이 고정이지만(§1.4) 팩트 라인에 계좌가 있으므로 계좌 필터가 성립한다.

축 값은 **논리적으로는 라인의 속성이지만, 물리적으로는 마스터 조인**으로 얻는다. `position_line`은 `account_id`·`instrument_id`만 갖고 섹터·시장·통화·자산군은 `instrument`/`account` 마스터에서 조인한다.

분류를 라인에 복사하면 오분류가 과거 스냅샷에 영구히 박혀 되돌릴 수 없다. 섹터 체계 통일 매핑은 아직 미해결이라 보정이 여러 번 예상되므로, **분류 보정이 과거 화면에도 소급 반영되는 쪽**을 택한다. 보유 규모상 조회 시 집계로 충분하므로 별도 집계 캐시는 두지 않는다.

### 3.4 렌즈

렌즈는 **입력도 라인 집합, 출력도 라인 집합인 변환 함수**다. 출력 스키마가 입력과 동일하므로 하위 집계·비중·환산 로직은 변경되지 않는다.

```
DIRECT       : [TIGER 미국나스닥100  평가 512,000]
                     │ lens = LOOK_THROUGH
                     ▼ ETF 1행 → 구성종목 N행으로 안분 (총합 보존)
LOOK_THROUGH : [AAPL 148,000] [MSFT 121,000] … [기타 16,000]
```

안분 시 발생하는 반올림 잔차는 기타 버킷에 흡수해 총합을 맞춘다.

`etf_coverage.state = UNAVAILABLE`인 ETF는 **전개하지 않고 ETF 행을 그대로 남긴다.** 기타 버킷에 넣으면 "ETF 내 비주식"과 뭉개진다. 미분해 평가금액을 `CONSTITUENT_UNAVAILABLE` notice에 실어 사용자가 분해되지 않은 몫을 알게 한다.

중첩 ETF(ETF가 다른 ETF를 보유)는 **평탄화된 구성비중으로 제공받는다.** 재귀 분해·깊이 제한·순환 참조 차단은 구성비중 산출 단계에서 처리되므로, 전개는 `ETF 평가금액 × 최종 구성비중` 한 겹 곱셈으로 끝난다.

전개는 **조회 시 수행하고 항상 최신 구성비중을 사용한다.** 구성비중이 갱신되면 화면 값도 따라 바뀐다(§3.3의 소급 반영과 동일 원칙). 전개 결과나 적용한 구성비중 버전은 저장하지 않는다.

**렌즈는 뷰의 속성이 아니라 지표의 속성이다.** 금액 기반 구성 비중을 다루는 곳에만 붙는다. 총자산·총손익·계좌 합계는 렌즈를 걸어도 값이 변하지 않으므로(총합 보존), 해당 뷰에는 렌즈를 노출하지 않는다.

**분해 후 컬럼별 유효성**

| 컬럼 | LOOK_THROUGH | 근거 |
|---|---|---|
| 평가금액 · 비중 | 행·합계 모두 유효 | 총합 보존 |
| 수량 | 행에 환산수량으로 표시 | `(ETF 평가금액 × 구성비중) ÷ 구성종목 현재가`. 소수점이며 개별 매도 불가 → "환산" 표기 |
| 매입금액 · 평가손익(금액/률) | **합계만 유효 — 행에서 제외** | 안분 원가는 사용자가 그 가격에 매수한 적 없는 값이다. 총합은 보존되므로 합계 수준에서는 성립 |
| **평단가** | **정의 불가 — 컬럼 숨김** | 아래 참조 |

행에 남는 것은 **평가금액 · 비중 · 환산수량** 셋뿐이다. 허구인 값을 "안분" 배지로 라벨링해 노출하는 것보다 행에서 빼는 쪽이 일관된다.

**평단가가 원리적으로 성립하지 않는 이유**는 데이터 부족이 아니라 정의 자체의 문제다.

1. 매입 시점의 구성비중이 필요하나 **구성비중 시계열은 공개되지 않는다**(운용사는 최신 파일만 배포).
2. ETF는 계속 리밸런싱되므로 **보유 기간 중 구성종목이 변동**한다. 매수한 적 없는 종목이 현재 구성에 있을 수 있고, 매수 당시 있던 종목이 지금은 빠졌을 수 있다.
3. 분할 매수 시 **회차마다 구성이 다르다**.

현재 비중으로 ETF 원가를 안분한 값은 계산할 수 있으나, 사용자가 그 가격에 매수한 적이 없는 허구의 숫자다. 평단이 허구이면 평가손익률도 허구가 된다.

**분해 결과는 근사값이다.** 잔고 스냅샷의 기준시각과 ETF 구성비중 파일의 기준일이 어긋나므로(구성비중은 전일자 등), 분해된 평가금액과 비중은 그 시차만큼 오차를 포함한다. 렌즈를 켠 화면에는 **두 기준시각을 함께 표기**한다.

```
기준 2026-07-27 15:30 · 구성비중 기준 2026-07-26
```

### 3.5 뷰 사양

뷰는 코드가 아니라 사양 데이터다.

```json
{ "view_key": "allocation",
  "axis": "sector",
  "lens": "LOOK_THROUGH",
  "metrics": ["market_value_krw", "weight_pct"],
  "filters": { "account_type": "PENSION" } }
```

"국내/미국 비중"은 `group_by: ["market"]`, "연금계좌만 섹터 비중"은 필터 추가로 끝난다. 뷰 추가에 API·테이블·파이프라인이 늘지 않는다.

### 3.6 처리 파이프라인

배치는 `position_line` · `position_basis` · `realized_pnl_line` 셋을 만들고, 렌즈 전개부터는 조회할 때마다 수행한다.

| 단계 | 하는 일 | 입력 → 출력 | 시점 | 담당 |
|---|---|---|---|---|
| 0 | 잔고·구성종목 수집, 종목 마스터 갱신, 구성비중 중첩 평탄화 | 증권사 API · 운용사 파일 → 원본 · 마스터 | EOD 배치 | 데이터 |
| 1 | 잔고를 계좌×종목 행으로 정규화. **매입금액은 잔고 평단 기준**, 평가금액은 현재가 기준. 원화 환산, 예수금을 `CASH` 종목으로 편입, 실패 계좌 이월 | 원본 잔고 · 환율 → `position_line` | EOD 배치 · 수동 동기화 | 백엔드 |
| 1.5 | 기업행위 보정, `opening_qty` 역산, 등급 판정 | `position_line` · `cln_trade` · `corporate_action` → `position_basis` | EOD 배치 | 백엔드 |
| 1.6 | 매도 체결마다 실현손익 산출 | `cln_trade` · `position_basis` · `fx_rate` → `realized_pnl_line` | EOD 배치 | 백엔드 |
| 2 | ETF 행을 구성비중대로 안분. 미매칭·비중 미달분을 기타 버킷으로. 총합 보존 검증 | `position_line` · 구성비중 → 전개 라인 | 조회 시 | 백엔드 |
| 3 | 종목·계좌 마스터를 조인해 축 값 부여 | 전개 라인 · 마스터 → 축이 붙은 라인 | 조회 시 | 백엔드 |
| 3.5 | 요청 필터 적용 | 축이 붙은 라인 → 대상 라인 | 조회 시 | 백엔드 |
| 4 | 요청 축으로 `GROUP BY` + 측정값 `SUM` | 축이 붙은 라인 → 축 값별 합계 | 조회 시 | 백엔드 |
| 5 | 평가손익 · 손익률 · 비중 계산 | 축 값별 합계 → 파생 지표 | 조회 시 | 백엔드 |
| 6 | 통화 선택, 평가금액순 정렬, 기타 버킷을 맨 끝에 배치, as-of · 구성비중 기준일 첨부 | 파생 지표 → 응답 | 조회 시 | 백엔드 |

- `DIRECT` 렌즈는 2단계를 건너뛰고 `position_line`이 그대로 3단계로 간다.
- 같은 종목의 직접보유분과 ETF 경유분은 4단계에서 합쳐진다.
- 실현손익 뷰는 `realized_pnl_line`을, 자산 변화 뷰는 `position_line` 두 시점과 `cln_cashflow`를 읽으므로 2~3단계를 거치지 않는다.
- 필터는 마스터 조인 뒤에 적용한다. 계좌 필터는 전개가 `account_id`를 보존하므로 2단계 앞으로 밀어도 결과가 같으며, 이는 구현 최적화 여지로 남긴다.
- **`position_basis`는 전량 재계산한다.** 증분 누적하면 기업행위나 과거 체결이 뒤늦게 도착했을 때 되돌릴 수 없다. 계좌×종목 단위로 확보 구간 전체를 매번 다시 접는다. 재계산 대상은 현재 열린 포지션의 확보 구간이며, 청산으로 리셋된 과거 구간은 재현하지 않는다.
- `realized_pnl_line`은 PK가 `trade_id`라 upsert로 멱등하다. 재계산 범위에 드는 행만 갱신되고 나머지는 그대로다.
- `LOOK_THROUGH`에서는 `lens_sensitive` 축으로 필터할 수 없다(§9.3). 전개가 종목 자체를 바꾸므로 전개 전후 어느 순서로 걸어도 답이 성립하지 않는다 — 자산군=ETF 필터는 전개 후 ETF가 존재하지 않아 항상 빈 목록이 된다.

### 3.7 통화 표시

집계 값은 **항상 원화**로 표시한다. 통화가 다른 금액은 더할 수 없으므로(§1.5 가산성), 여러 통화가 섞일 수 있는 묶음에는 현지 통화 표시가 성립하지 않는다.

묶음 안에 통화가 하나뿐일 때만 현지 통화를 병기한다.

| 묶음 | 단일 통화 | 표시 |
|---|---|---|
| 종목 1행 | ✓ | 평가금액을 `$612.30 · 848,000원`으로 병기 |
| 통화 축 그룹 · 시장 축 그룹 | ✓ | 해당 통화로 표시 |
| 섹터 · 자산군 · 계좌 · 포트폴리오 전체 | ✗ | 원화만 |

이 표는 예시이고 **판정은 조회 시점에 "이 묶음이 단일 통화인가"로 한다.** 축 이름을 하드코딩하면 축이 늘 때마다 표를 고쳐야 한다. 시장 축이 ✓인 것도 정의상 단일 통화라서지 시장이라는 이름 때문이 아니다.

"미국 자산이 달러로 얼마인가"는 비중 분석에서 통화 축이나 시장 축을 선택해 확인한다.

### 3.8 사용자 스코프

여러 사용자가 각자 계좌를 연동하므로, 집계에 들어가기 전에 **대상 행이 누구 것인가**가 먼저 정해진다. 이것은 축도 필터도 아니다.

| 개념 | 정하는 것 | 사용자가 고르는가 | 그레인 |
|---|---|---|---|
| 축 | 결과를 어떻게 묶을지 | ○ | 결정한다 |
| 필터 | 어떤 행을 대상으로 할지 | ○ | 바꾸지 않는다 |
| **스코프** | 애초에 어떤 행이 존재하는지 | **✕ — 인증 주체가 정한다** | 바꾸지 않는다 |

**사용자를 축으로 두지 않는다.** 축이면 "사용자별 비중"이라는 집계가 성립해 버리는데, 남의 자산과 내 자산을 한 분모에 넣는 질문은 이 서비스에 없다. **필터로도 두지 않는다.** 필터는 사용자가 끄고 켤 수 있는 것이고 스코프는 끌 수 없어야 한다.

**소유권 축은 계좌 하나다.** `account.user_id`가 유일한 소유 표시이고, `position_line` · `realized_pnl_line` · `manual_cashflow`는 전부 `account_id`를 가지므로 계좌를 조인하면 소유자가 결정된다. 소유 사실을 라인에 복사하면 같은 사실이 네 곳에 놓여 어긋날 자리가 생긴다 — 분류를 라인에 복사하지 않는 것과 같은 이유다(§3.3).

**스코프는 파이프라인 2단계 앞에 선다.** §3.6의 2단계(렌즈 전개)가 받는 라인 집합이 이미 그 사용자의 것이어야, 아래의 조인·필터·집계 어느 단계도 스코프를 빠뜨릴 수 없다. 필터를 마스터 조인 뒤에 거는 것과 순서가 반대인데, 필터는 결과를 좁히는 일이고 스코프는 **대상 세계를 정하는 일**이라 그렇다.

**`as_of` 캘린더도 스코프 안에 있다.** 직전 영업일·기간 경계 스냅샷은 그 사용자의 계좌에 라인이 있는 `as_of`에서만 고른다. 연동을 늦게 시작한 사용자는 그 이전 `as_of`를 갖지 않으며, 남의 `as_of`를 기초로 잡으면 기간 손익이 남의 세계에서 계산된다.

**계좌 필터는 소유 검사를 거친다.** 요청의 계좌 값이 인증 주체의 계좌가 아니면 빈 결과를 내지 않고 거부한다(§9.3). 조용히 비우면 남의 계좌 ID가 존재하는지 여부가 응답 차이로 드러난다.

---

## 4. 3원장과 신뢰도 등급

### 4.1 원장별 역할

| 원장 | 그레인 | 답하는 것 | 확보 |
|---|---|---|---|
| **보유 스냅샷** `position_line` | (as_of, 계좌, 종목) | 현재 상태 전부 | 항상 확보 — 잔고가 진실 |
| **현금흐름** `cln_cashflow` · `manual_cashflow` | (계좌, 유형, 일시) | 자산 변화의 원인 | 유형별로 원천이 다름 (§5.1) |
| **거래** `cln_trade` | 체결 1건 | 실현/미실현 **분해** | 구간별 불완전 |

미실현 손익의 원가는 **잔고 평단**, 실현손익의 원가는 **재구성 평단**이며 **둘은 일치하지 않는다.** 증권사 잔고 평단은 매수 수수료를 포함하지 않는데 §4.3은 취득원가에 포함하기로 정했으므로, 차이는 모호한 값이 아니라 **정확히 매수 수수료 전액**이다. 여기에 `SEEDED` 구간의 재구성값이 추정이고 기업행위 이력이 없으면 어긋나는 것이 더해진다. 잔고 평단의 수수료 취급은 보정하지 않는다. `position_line`은 `position_basis`를 참조하지 않으며, 이 분리가 §4.2의 리스크 격리를 성립시킨다.

현재 보유 상태는 **증권사 잔고 조회가 유일한 진실의 원천**이며 거래내역에서 역산하지 않는다. 액면분할·증자·배당재투자에서 오차가 누적되고, 거래가 하나라도 누락되면 전체가 틀어지기 때문이다.

### 4.2 뷰별 원장 의존도

| 뷰 | 스냅샷 | 현금흐름 | 거래 | 등급 노출 |
|---|---|---|---|---|
| 요약 | ● | | | 없음 |
| 종목별 | ● | | | 없음 |
| 비중 분석 | ● | | | 없음 |
| 계좌별 | ● | | | 없음 |
| **실현손익** | | | ● | **있음** |
| **자산 변화** | ● | ● | ○ (분해 시만) | `split_available`로 표현 |

6개 뷰 중 4개가 **항상 신뢰 가능한 원장만** 사용한다. 거래내역 불완전 리스크는 실현손익 뷰 한 곳에 갇힌다.

### 4.3 평단과 실현손익

평단은 거래내역 기반 **이동가중평균**이며, `position_basis`의 그레인이 (계좌 × 종목)이므로 **계좌마다 독립**으로 계산한다.

**취득과 처분**

| 규칙 | 내용 |
|---|---|
| 취득원가 | 매수 수수료를 포함한다. `평단 = (기존 매입금액 + 매수금액 + 수수료) ÷ 총수량` |
| 매도 | `실현손익 = (매도단가 - 평단) × 수량 - 매도 수수료·세금` |
| 부분 매도 | 평단은 변하지 않는다. 수량과 매입금액만 비례해 줄어든다 |
| 청산 | 보유수량이 0이 되면 `position_basis`를 리셋한다. 재매수는 새 포지션으로 `VERIFIED`에서 시작한다 |
| 기간 귀속 | **체결일** 기준. 결제일(T+2)을 쓰지 않는다 |

청산 시 리셋은 등급 판정과 맞물린다. 수량이 0으로 확정된 지점 이후의 이력은 완전하므로, 과거의 `SEEDED`가 새 포지션까지 오염시키지 않는다.

**외화 종목**

취득원가는 매입일 환율로, 매도금액은 매도일 환율로 환산한다.

```
실현손익_krw = 매도금액 × 매도일 환율 - 취득원가 × 매입일 환율 - 비용
```

원화 실현손익에는 **환손익이 포함된다.** 미실현 평가손익은 양변을 같은 환율로 환산해 환손익이 제외되므로(§5.3) 둘의 취급이 다르다. 매입일·매도일 환율을 조회할 수 있도록 `FxRate`를 일별로 보관한다.

**기업행위 보정**

`ex_date` 시점에 **상태만 조정**하고 과거 거래내역은 재작성하지 않는다.

```
수량 = 수량 × ratio
평단 = 평단 ÷ ratio
```

분할이 매수와 매도 사이에 끼면 보정 없이는 평단이 부호까지 틀어진다 — 10주@100이 2:1 분할로 20주@50이 되는데 거래내역에는 매수 10@100만 남는다. 이력이 없으면 `ca_unknown` 플래그를 세운다(§4.4).

`realized_pnl_line`은 **산출 시점 상태의 스냅샷**이다. 금액과 등급 모두 그때의 `position_basis`로 계산하며, 위의 청산 리셋이나 등급 강등(§4.4) 이후에도 과거 행을 소급 수정하지 않는다. 분류 보정은 소급 반영하고(§3.3) 손익 산출은 시점을 고정하는 것이 이 스펙의 대비다.

**불완전 거래내역 보정**

거래내역이 포지션 개시 시점부터 확보되지 않으면 잔고 스냅샷의 평단을 출발 평단(seed)으로 삼는다.

**세무 신고용이 아니다.** 수수료·세금 처리는 필요경비 관행을 따르지만 평단을 계좌마다 독립으로 계산하므로, 과세 대상 개인 단위로 합산하는 양도소득세의 양도차익과 일치하지 않는다.

**증권사 화면 수치와도 다를 수 있다.** 증권사는 매수 수수료를 매수 시점의 실현손실로 인식하기도 하는데, 이 스펙은 취득원가에 포함해 매도 시점까지 이연한다. 실현손익을 "기간 매도 확정 손익"으로 정의한 §2.8과 일관되게 하기 위해서다. 매수만 하고 매도가 없는 기간에는 증권사 화면과 부호가 갈린다.

### 4.4 신뢰도 등급 — 계산 가능한 판정

등급은 감으로 붙이지 않고 **(계좌 × 종목)마다 역산해서 판정**한다.

```
opening_qty = position_line.quantity − Σ(확보구간 매수) + Σ(확보구간 매도)   ← 기업행위 보정 후
```

확보된 거래내역을 **잔고 수량**에서 거꾸로 되감았을 때 시작 시점 수량이 0이면, 그 구간이 곧 포지션 전체다. 좌변은 반드시 잔고여야 한다 — 역산의 목적이 잔고와 거래를 대조하는 것이므로, 거래에서 재구성한 수량을 넣으면 자기참조가 되어 `opening_qty`가 항상 0이 되고 모든 포지션이 `VERIFIED`로 판정된다.

**판정 시점을 고정한다.** 등급은 파이프라인 1단계 직후 배치에서만 판정하고 조회 시 재판정하지 않는다. 역산에 쓰는 체결은 `executed_at <= as_of`로 자르며, 사용한 잔고 시점과 수량을 `evaluated_as_of` · `evaluated_qty`에 기록한다.

| 등급 | 판정 | 실현손익 | UI |
|---|---|---|---|
| **VERIFIED** | `opening_qty = 0` | 신뢰 | 배지 없음 |
| **SEEDED** | `opening_qty > 0` → 잔고 평단을 시드로 보정 | 추정값 | **"추정" 배지** |
| **UNAVAILABLE** | 거래내역 미확보 | 미제공 | "거래내역 없음" |
| **CONFLICT** | `opening_qty < 0` (수량 모순) | 미제공 | "거래내역이 아직 도착하지 않았을 수 있습니다" |

**직교 플래그** `ca_unknown`: **보유 기간이 `ca_coverage` 밖에 걸치면** `VERIFIED`여도 경고를 덧붙인다. 등급과 마찬가지로 감이 아니라 커버리지 대조로 판정한다. 등급은 "거래가 다 있는가", 플래그는 "그 거래를 올바로 해석했는가"를 판정하므로 성격이 다르다.

**시차로 인한 일시적 오판을 전제한다.** 잔고와 체결은 도착 시점이 달라, 매수 직후 체결이 아직 안 왔으면 `SEEDED`, 반대 순서면 `CONFLICT`가 된다. 정상 매매 한 번이 경고를 띄울 수 있으므로 문구를 재동기화 유도가 아니라 도착 지연 안내로 둔다.

**고칠 수 없는 경고를 영구 표시하지 않는다.** `CONFLICT`가 구조적 원인으로 판정되어 다음 판정에서도 동일하게 재현되면 `UNAVAILABLE`로 강등해 조용히 제외하고 `미포함 계좌 N개`에 흡수한다(§2.8). 사용자가 해소할 수 없는 상태를 계속 노출하지 않기 위해서다.

**같은 원칙을 `SEEDED`에도 적용한다.** 체결을 거치지 않은 입고(타사대체·현물 이관)로 생긴 `SEEDED`는 재동기화나 조회 구간 확대로 해소되지 않는다. 확보 구간이 계좌 최초 체결일까지 닿았는데도 `opening_qty > 0`이면 **영구 `SEEDED`**로 판정하고, 배지 문구에서 "곧 해소될 수 있다"는 뉘앙스를 뺀다.

**확보 구간 중간의 미설명 변동도 본다.** 역산은 구간의 양 끝만 보므로, 구간 안에서 체결 없이 수량이 변하면 상쇄되어 드러나지 않거나 반대로 `CONFLICT`로 뜨면서 원인이 오인된다. 기준일별 보유수량을 제공하는 원천이 있으면 대조해 미설명 변동을 별도로 표시한다.

### 4.5 SEEDED 표시 정책

`SEEDED`는 두 원인에서 생기며 성격이 다르다.

| 원인 | 해소 |
|---|---|
| 거래내역 확보 구간이 포지션 개시보다 늦음 | 조회 구간을 넓히면 해소 |
| **매매로 취득되지 않은 포지션** — 타사대체·현물 이관 등 | 해소되지 않음 (영구 `SEEDED`) |

숨기지 않고 **표시하되 배지로 구분**한다.

- 종목 행에 `추정` 배지
- 합계 하단에 `N개 종목은 추정치입니다`
- 배지 탭 시 사유 설명 — 위 두 원인 중 어느 쪽인지와, 확보 구간이면 시작일

### 4.6 현금흐름의 커버리지

현금흐름은 **유형마다 원천이 달라** 확보 여부도 유형별로 갈린다. 판정 단위는 **(계좌, 유형)**이다.

| 유형 | 원천 |
|---|---|
| `DIVIDEND` | 증권사 권리·배당 조회 |
| `FEE` · `TAX` (매매 외) | 증권사 제공 여부에 따라 다름 |
| `DEPOSIT` · `WITHDRAW` | **사용자 입력** (§5.1 `manual_cashflow`) |

증권사가 입출금 이력 API를 제공하지 않는 경우가 있어, 입출금은 원천이 아니라 사용자 입력을 기본 경로로 둔다. 기간 조회 한계가 아니라 **원천 자체가 없는 것**이므로 기간 기반 판정이 성립하지 않는다.

미확보분은 투자손익에 섞이므로(§2.9에서 투자손익은 나머지 전부로 정의됨) 자산 변화 뷰에 경고를 표시한다.

```
7월 입출금이 입력되지 않아 투자손익에 섞여 있을 수 있어요
```

잔차 항목은 두지 않는다. 투자손익이 나머지 전부로 정의되어 항등식에 잔차가 생길 자리가 없다.

---

## 5. 데이터 모델

### 5.1 테이블

소유는 **기관 고유성이 사라지는 지점**을 경계로 나눈다. 기관마다 다른 응답을 흡수해 공통 모양으로 만드는 것까지가 데이터, 거기에 도메인 규칙(평단·기업행위 보정·캐리포워드·등급 판정)을 얹는 것이 백엔드다. 물리 테이블명은 팀 규약을 따른다.

#### 참조 데이터 — 데이터 소유

**`instrument`** 종목 마스터

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `instrument_id` | uuid PK | |
| `isin` | text | 국내·미국 공통 키 |
| `symbol` | text | 종목코드 / ticker |
| `name` | text | |
| `asset_class` | enum | `STOCK` · `ETF` · `CASH` |
| `market` | enum | `KR` · `US` |
| `currency` | enum | `KRW` · `USD` |
| `sector` | text null | 분류 미확정 시 null |
| `is_leveraged` | bool null | 원천 확보 전 null |

**`etf_constituent`** 구성비중 — 중첩이 이미 펼쳐진 한 겹

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `as_of` | date | 구성비중 파일 기준일 |
| `etf_instrument_id` | uuid | |
| `underlying_instrument_id` | uuid | 최종 종목 |
| `weight` | decimal(9,6) | 합이 100% 미만일 수 있음 → 기타 버킷 |

PK `(as_of, etf_instrument_id, underlying_instrument_id)`

**`fx_rate`** 일별 환율 · **`corporate_action`** 기업행위

| 테이블 | 컬럼 |
|---|---|
| `fx_rate` | `as_of` date · `currency_pair` text · `rate` decimal(18,6) — PK `(as_of, currency_pair)` |
| `corporate_action` | `instrument_id` · `type` enum(`SPLIT`·`RIGHTS`·`MERGER`) · `ratio` decimal · `ex_date` date |
| `ca_coverage` | `market` 또는 `instrument_id` · `covered_from` · `covered_to` |
| `etf_coverage` | `etf_instrument_id` · `state` enum(`COVERED`·`UNAVAILABLE`) · `as_of` |

**"없음"과 "미확보"를 구분하는 것이 커버리지 테이블의 존재 이유다.** 행이 없다는 사실만으로는 기업행위가 없었던 것인지 데이터를 못 구한 것인지 알 수 없다. 구분하지 못하면 원천 미확보 시 모든 종목이 "정황 없음"이 되어 `ca_unknown`이 하나도 뜨지 않고, 틀린 실현손익이 `VERIFIED` 배지를 달고 나간다.

#### 정규화 원본 — 데이터 소유

기관별 응답 차이를 흡수한 결과이며 백엔드는 읽기만 한다. `raw_*`는 기관 응답 원문 보관용이며 백엔드는 읽지 않는다.

| 테이블 | 컬럼 |
|---|---|
| `cln_balance` | `account_ref` · `isin` · `quantity` decimal(20,8) · `avg_price` decimal · `market_value` decimal · `currency` · `source_as_of` timestamptz |
| `cln_deposit` | `account_ref` · `currency` · `amount` decimal · `source_as_of` |
| `cln_trade` | `trade_id` · `account_ref` · `isin` · `side` enum(`BUY`·`SELL`) · `quantity` · `price` · `fee` · `tax` · `currency` · `executed_at` timestamptz |
| `cln_cashflow` | `account_ref` · `type` enum(`DIVIDEND`·`FEE`·`TAX`) · `amount` · `currency` · `occurred_at` timestamptz |
| `collection_run` | 수집 실행·트리거 상태 (§7.7) |

`cln_cashflow`는 **계좌에 귀속되는 손익성 현금만** 담는다. 증권사가 입출금 이력을 제공하더라도 매수대금 출금·매도대금 입금이 같은 응답에 섞여 오므로, 그런 원천을 쓸 때는 데이터가 걸러낸다. 매매대금이 현금흐름으로 들어오면 매도 한 번에 그 금액만큼 손실이 표시되어 §2.9 항등식이 붕괴한다.

| 배제 | 사유 |
|---|---|
| 매매대금 입출금 | 주식과 예수금 사이의 이동이라 총자산이 변하지 않는다 |
| 예수금 내부 이동 | RP·CMA 자동매수/환매 |
| 환전 | 통화 간 이동이라 원화 총자산이 변하지 않는다 |
| 매매 수수료·거래세 | 체결가에 반영되어 실현손익 계산에 이미 쓰인다(§4.3). `FEE`/`TAX`는 계좌 관리수수료·환전수수료·배당소득세 등 **매매 외 비용만** |

`DIVIDEND`는 원천이 준 금액을 그대로 쓴다. 원천징수 후 실입금액이면 그대로 두고 세전 환원을 시도하지 않는다 — 투자손익이 나머지 전부로 정의되므로 자동으로 정합한다.

**계좌 경계를 넘는 자금 이동(`DEPOSIT`·`WITHDRAW`)은 사용자 입력으로 받는다.** 증권사 API가 입출금 이력을 열어주지 않는 경우가 있어 원천에 기댈 수 없다. 입력 빈도가 월 1~2건으로 낮고 사용자가 이체 기록으로 정확한 값을 아는 데이터라, 자동 연동만 지원한다는 원칙(§12)의 예외로 둔다.

#### 도메인 — 백엔드 소유

**`app_user`** 사용자

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `user_id` | uuid PK | 스코프의 기준 (§3.8) |
| `email` | text UNIQUE | 로그인 ID |
| `password_hash` | text | BCrypt. 원문은 어디에도 저장하지 않는다 |
| `display_name` | text | 화면 표시명 |
| `created_at` | timestamptz | |

행은 마이그레이션이 심는다. 회원가입 경로를 두지 않는다(§12).

**`account`** 계좌

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `account_id` | uuid PK | |
| `user_id` | uuid | 소유자. **생성 후 바뀌지 않는다** — 계좌 소유자 이전을 지원하지 않는다 |
| `account_ref` | text UNIQUE | 데이터팀에 넘기는 불투명 문자열(계좌번호 아님). 같은 계좌면 재연동해도 같은 값 (§7.1 · §11.2) |
| `broker` | text | 기관명 |
| `label` | text | 표시명. 같은 기관에 계좌가 여럿일 수 있어 `broker`와 별개로 둔다 |
| `account_type` | enum | `GENERAL` · `PENSION` |
| `source` | enum | `KIS` · `CODEF` |
| `credential_ref` | text null | 시크릿 매니저 키. KIS 앱키·CODEF Connected ID 공용. 값 자체는 저장하지 않음 (§7.1) |
| `link_state` | enum | 연동 상태만. `CONNECTING`·`CONNECTED`·`REAUTH_REQUIRED`·`DISCONNECTED` (§7.2) |
| `last_synced_at` | timestamptz null | |

**`position_line`** 보유 스냅샷 — 하루 한 벌

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `as_of` | date | 일자 단위. 당일 upsert, 과거 불변 |
| `account_id` · `instrument_id` | uuid | |
| `quantity` | decimal(20,8) | 소수 허용 |
| `cost_amount_local` · `market_value_local` | decimal(20,4) | 매입금액은 `cln_balance.avg_price × quantity`. `position_basis`를 쓰지 않는다 |
| `cost_amount_krw` · `market_value_krw` | decimal(20,0) | 원화는 정수 |
| `fx_rate` | decimal(18,6) | `market_value_krw`가 있으면 필수 |
| `fx_as_of` | date | |
| `source_as_of` | timestamptz | 계좌별 실제 데이터 시각 |
| `is_carried_forward` | bool | |
| `is_final` | bool | EOD 배치만 true. 수동 동기화가 덮지 않는다 |

PK `(as_of, account_id, instrument_id)`

**`position_basis`** 평단·등급 상태

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `account_id` · `instrument_id` | uuid | PK |
| `evaluated_as_of` | date | 판정에 사용한 잔고 시점 |
| `evaluated_qty` | decimal(20,8) | 그때의 `position_line.quantity` |
| `avg_cost_local` · `avg_cost_krw` | decimal(20,4) | 매수 시점 환율로 누적 |
| `coverage_start_at` | date null | 거래내역 확보 시작일 |
| `opening_qty` | decimal(20,8) | 역산 결과 (§4.4) |
| `seed_avg_price` | decimal null | `SEEDED`일 때 필수 |
| `grade` | enum | `VERIFIED`·`SEEDED`·`UNAVAILABLE`·`CONFLICT` |
| `ca_unknown` | bool | 등급과 직교 |

청산 시 리셋한다(§4.3).

**`realized_pnl_line`** 실현손익 — 매도 체결 1건

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `trade_id` | text PK | `cln_trade` 참조 |
| `account_id` · `instrument_id` | uuid | |
| `sold_at` | timestamptz | 기간 귀속은 체결일 |
| `quantity` | decimal(20,8) | |
| `sell_amount_local` · `cost_basis_local` | decimal(20,4) | |
| `sell_amount_krw` · `cost_basis_krw` | decimal(20,0) | 각각 매도일·매입 시점 환율 적용 |
| `fee_tax` | decimal(20,4) | |
| `realized_pnl_local` | decimal(20,4) | |
| `realized_pnl_krw` | decimal(20,0) | |
| `grade` | enum | `position_basis`에서 상속 |

**`manual_cashflow`** 사용자 입력 입출금

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `id` | uuid PK | |
| `account_id` | uuid | |
| `type` | enum | `DEPOSIT` · `WITHDRAW` |
| `amount` | decimal(20,0) | |
| `currency` | enum | |
| `occurred_on` | date | 사용자가 지정 |
| `memo` | text null | |

**`sync_run`** 동기화 실행

| 컬럼 | 타입 |
|---|---|
| `run_id` uuid PK · `account_id` uuid **null**(전체 동기화는 null) · `started_at` · `finished_at` null · `state` enum(`RUNNING`·`PARTIAL`·`DONE`·`FAILED`) · `failure_reason` text null |

사용자에게 보여줄 진행 상태 전용이다. 전 계좌 새로고침은 부모 `sync_run` 1행과 계좌별 `collection_run` N행으로 구성된다.

> `Portfolio`는 테이블이 아니라 `position_line` 집계에서 파생된다.

### 5.2 예수금을 종목으로 취급한다

`instrument.asset_class = CASH`인 통화별 의사종목(`KRW 예수금`, `USD 예수금`)을 두고, 예수금도 `position_line`에 넣는다. `cln_deposit`을 이 의사종목 행으로 바꾸는 것은 백엔드가 한다.

이렇게 하면 **총자산·현금비중·자산군 축이 모두 같은 팩트 하나에서 나온다.** 예수금을 별도 테이블로 분리하면 "총자산 = 평가 + 예수금"을 화면마다 다시 조립해야 하고, 자산군 비중 차트에서 현금만 특수 처리하게 된다.

**CASH 행의 원가는 평가금액과 같은 값으로 저장한다.** `null`을 두면 `fx_rate` 필수 규칙(§9.1)과 충돌하고 집계에서 NULL이 전파된다. 대신 **손익 계열 지표는 집계에서 `asset_class != CASH` 조건을 적용**해 예수금이 분모에 섞이지 않게 한다. 저장값과 노출값이 다르며, 응답의 CASH 행은 원가·손익을 `null`로 내린다.

### 5.3 원화 환산은 라인에 머티리얼라이즈한다

`market_value_krw`를 라인 생성 시점에 계산해 저장하고 `fx_rate`·`fx_as_of`를 함께 기록한다. 집계 시점에 환율을 곱하면 여러 통화가 섞인 그룹에서 어떤 환율을 적용할지 모호해지지만, 라인 단위에서 환산해두면 단순 합산으로 끝난다.

**이것이 '환율 기준시점 관리' 요구사항의 구현 전부다** — 별도 기능이 아니라 컬럼 두 개.

환율이 결측이면 **직전 영업일 환율로 폴백**하고 `fx_as_of`가 그 날짜를 가리킨다. `fx_as_of < as_of`가 곧 폴백의 증거이므로 별도 플래그를 두지 않으며, `FX_APPLIED` notice가 최고령 `fx_as_of`를 싣는 덕에 화면 고지도 자동으로 붙는다. 폴백이 5영업일을 넘으면 `FX_STALE` notice로 통화쌍을 표시한다. 잔고는 최신이므로 캐리포워드하지 않는다.

`cost_amount_krw`도 같은 `fx_rate`로 환산하므로 **원화 평가손익에는 환손익이 포함되지 않는다.** 가격손익을 평가시점 환율로 환산한 값이다.

환율 변동의 영향은 총자산에 반영되고 자산 변화 뷰의 투자손익이 답한다(§2.9). 평가손익은 "이 종목이 얼마나 올랐나", 투자손익은 "환율까지 포함해 얼마나 벌었나"로 질문이 갈린다.

| | 원가 | 환손익 |
|---|---|---|
| 미실현 평가손익 | 잔고 평단 | 제외 |
| 실현손익 | 재구성 평단(매입일 환율 누적) | 포함 |
| 자산 변화 투자손익 | Δ총자산 기반 | 포함 |

### 5.4 스냅샷 누적

`position_line`은 **국내 증시 영업일마다** 한 벌씩 쌓인다. 주말·공휴일에는 만들지 않는다. 캘린더일로 만들면 주말 라인이 전부 캐리포워드가 되어 월요일마다 지연 배너가 오작동한다.

연동을 늦게 시작한 사용자는 그 이전 `as_of`를 갖지 않는다. `as_of`는 전 사용자 공통의 영업일이지만 **어느 날짜가 그 사용자에게 존재하는가는 계좌가 정한다**(§3.8).

`as_of`는 **그 날짜의 배치가 관측한 세계의 상태**다. 한국 EOD 시점에 미국 시장은 아직 열리지 않았으므로 미국 종목은 그때 조회 가능한 최신 종가(직전 미국 영업일)를 쓴다. 나중에 나온 미국 종가로 과거 행을 고치지 않는다 — 고치면 자산 변화와 일간 변화가 재현 불가능해진다. 시장별 가격 기준일은 `PRICE_LAG_MARKET` notice로 고지한다(§8.2).

**확정과 잠정을 `is_final`로 가른다.** EOD 배치만 `is_final = true`로 쓰고, 그 행은 이후 수동 동기화가 덮지 않는다. 새로고침은 `collection_run`을 돌려 `last_collection`을 갱신하되 라인은 그대로 두고 "이미 확정된 값입니다"를 알린다. 당일 `is_final = false` 행은 동기화할 때마다 갱신한다. 과거 `as_of` 행은 어느 경우에도 수정하지 않는다.

자산 변화 뷰가 기초자산을 꺼내려면 과거 스냅샷이 필요하고, 요약 뷰의 일간 변화도 전일 스냅샷에 의존한다. 부수 효과로 자산 추이 시계열이 같은 저장소에서 나온다.

### 5.5 타입 정책

| 항목 | 타입 | 근거 |
|---|---|---|
| 수량 | 소수 허용 | look-through 환산수량이 소수다 |
| 금액 | 통화별 소수 자릿수 유지. 원화 라인은 정수 | 반올림 위치가 총합 보존에 영향을 준다 |
| 환율 | 소수 4자리 이상 | |
| 비중 · 수익률 | 컬럼 없음 | §1.5 |

반올림은 **라인 단위에서 한 번만** 한다. 환산·안분한 값을 라인에 저장하고, 집계는 저장된 값을 그대로 더한다. 집계 후 재반올림하지 않는다.

---

## 6. 카탈로그

뷰가 6개로 고정이고 운영자가 런타임에 뷰를 추가할 일이 없으므로 **코드 상수**로 둔다. DB 테이블이 아니다.

### 6.1 축

```
Axis { key, label, source, applicable_views[], lens_sensitive }
```

| key | 라벨 | 출처 | 사용 뷰 | 렌즈 영향 |
|---|---|---|---|---|
| `account` | 계좌 | 계좌 마스터 | 계좌별 | 없음 |
| `account_type` | 계좌유형 | 계좌 마스터 | 계좌별 | 없음 |
| `instrument` | 종목 | 종목 마스터 | 종목별 · 비중 분석 | 받음 |
| `sector` | 섹터 | 종목 마스터 | 비중 분석 | 받음 |
| `market` | 시장 | 종목 마스터 | 비중 분석 · 요약 미니차트 | 받음 |
| `currency` | 통화 | 종목 마스터 | 비중 분석 | 받음 |
| `asset_class` | 자산군 | 종목 마스터 | 비중 분석 · 요약 미니차트 | 받음 |
| `is_leveraged` | 레버리지 | 종목 속성 | 비중 분석 | 받음 |

**사용자는 이 표에 없다.** 축은 한 사람의 자산을 묶는 기준이고 사용자는 대상 행 집합 자체를 정하므로 성격이 다르다(§3.8).

계좌 축이 렌즈 영향을 받지 않는 이유는 look-through가 총합을 보존하기 때문이다. 그래서 계좌별 뷰에는 렌즈를 노출하지 않는다(§2.7). `is_leveraged`는 원천 확보 전까지 비활성이다.

**분류 축의 폴백**

| 대상 | 값 |
|---|---|
| CASH 의사종목 | 모든 분류 축에서 전용 값 `현금` |
| 분류가 없는 종목(`sector = null` 등) | `미분류` |

**자산군 값을 다른 축에 넣지 않는다.** 섹터 축 `DIRECT`에서 ETF는 섹터가 비어 있으므로 `미분류`로 모이며, 실제 섹터를 보려면 렌즈를 켜라는 안내를 붙인다. ETF를 섹터 값으로 쓰면 축이 오염되고 §3.4가 정한 렌즈의 역할과 겹친다.

### 6.2 지표

```
Metric { key, label, additive, cash_included, lens_safe, formula, requires_ledger }
```

| key | 라벨 | 가산 | CASH | 렌즈 | 계산 |
|---|---|---|---|---|---|
| `quantity` | 수량 | ○ | 포함 | `ROW_AND_TOTAL` | `Σ 수량` |
| `total_assets_krw` | 총자산 | ○ | **포함** | `ROW_AND_TOTAL` | 합계 전용. `Σ 평가금액` |
| `securities_value_krw` | 유가증권 평가금액 | ○ | **제외** | `ROW_AND_TOTAL` | 합계 전용. `Σ 평가금액` |
| `market_value_krw` | 평가금액 | ○ | 행 기준 | `ROW_AND_TOTAL` | **행 전용.** 그 행의 평가금액 |
| `deposit_krw` | 예수금 | ○ | 포함 | `ROW_AND_TOTAL` | `Σ CASH 평가금액` |
| `daily_change_krw` | 일간 변화 | ○ | 포함 | — | 당일 − **직전 `as_of`**의 `total_assets_krw` |
| `daily_change_pct` | 일간 변화율 | ✕ | 포함 | — | `daily_change_krw ÷ 직전 as_of의 total_assets_krw` |
| `realized_pnl_pct` | 실현손익률 | ✕ | 제외 | `NEVER` | `Σ실현손익 ÷ Σ취득원가` |
| `cost_amount_krw` | 매입금액 | ○ | **제외** | `TOTAL_ONLY` | `Σ 매입금액` |
| `unrealized_pnl_krw` | 평가손익 | ○ | **제외** | `TOTAL_ONLY` | `Σ평가 - Σ매입` |
| `unrealized_pnl_pct` | 평가손익률 | ✕ | **제외** | `TOTAL_ONLY` | `평가손익 ÷ Σ매입` |
| `realized_pnl_krw` | 실현손익 | ○ | 제외 | `NEVER` | `Σ 실현손익` |
| `avg_cost` | 평단 | ✕ | 제외 | `NEVER` | `Σ매입 ÷ Σ수량` — 잔고 평단·잔고 수량 기준 |
| `weight_pct` | 비중 | ✕ | 분모 포함 | `ROW_AND_TOTAL` | `행 평가금액 ÷ total_assets_krw` |
| `cash_ratio_pct` | 현금비중 | ✕ | 분모 포함 | `ROW_AND_TOTAL` | `CASH 평가 ÷ total_assets_krw` |
| `instrument_count` | 종목수 | ✕ | 제외 | `ROW_AND_TOTAL` | `COUNT DISTINCT 종목` |
| `account_count` | 계좌수 | ✕ | 포함 | — | `COUNT DISTINCT 계좌` |

**가산** — ○ 은 행을 더해도 되는 값, ✕ 는 집계 결과에만 적용 가능하며 라인 단위로 더하면 틀리는 값이다(§1.5).

**행 키와 합계 키는 이름이 겹치지 않는다.** 같은 키가 행에서는 CASH를 포함하고 합계에서는 제외하면, 소비자가 `Σ rows`와 `total`을 대조했을 때 어긋난다. 합계는 `total_assets_krw`·`securities_value_krw`로 갈라 각 키가 한 의미만 갖게 한다.

**CASH** — 예수금 의사종목을 집계에 포함하는지. 자산 배분 질문(총자산·비중·현금비중)은 포함하고, 손익 질문(매입금액·평가손익)은 제외한다. 제외하지 않으면 예수금이 손익률 분모에 섞여 값이 희석된다. **비중의 분모는 `total_assets_krw`(CASH 포함), 손익률의 분모는 `cost_amount_krw`(CASH 제외)로 서로 다르다.**

**렌즈** — `LOOK_THROUGH`에서의 유효 범위.

| 값 | 의미 |
|---|---|
| `ROW_AND_TOTAL` | 행과 합계 모두 유효 |
| `TOTAL_ONLY` | 합계에만 싣고 `rows[]`에서 제외. 안분 원가가 허구이므로 행 단위로는 의미가 없으나 총합은 보존된다(§3.4) |
| `NEVER` | 분해 결과에서 제공하지 않음 |

평가손익은 파생 지표이지만 가산 가능하다. 저장하지 않는 이유는 가산 불가여서가 아니라 중복 저장을 피하기 위해서다.

### 6.3 뷰

```
View { view_key, question, grain, group_by[], metrics[], row_fields[],
       filters: { DIRECT: [...], LOOK_THROUGH: [...] },
       lens_policy: NONE | OPTIONAL | ALWAYS,
       sub_blocks[], ledgers[] }
```

- `row_fields[]` — 공통 행 스키마(§8.3)에 더해지는 필드. 계좌별 뷰만 `link_state` · `last_collection`을 갖는다.
- `filters` — 렌즈 상태별 허용 필터 맵. `LOOK_THROUGH`에서는 `lens_sensitive` 축이 빠진다(§9.3).
- `sub_blocks[]` — 한 응답에 `group_by`·렌즈 조합이 둘 이상 필요한 뷰. 요약만 해당하며 `lens_policy`는 뷰 전체가 아니라 이 블록에 붙는다.

| view_key | 그레인 | group_by | 지표 | 필터 | 렌즈 | 원장 |
|---|---|---|---|---|---|---|
| `summary` | 전체 1행 | `[]` | 총자산 · 유가증권 평가금액 · 매입금액 · 평가손익(률) · 현금비중 · 일간 변화(금액/률) · 계좌수 · 종목수 | — | `NONE` (미니차트 `sub_block`에 `OPTIONAL`) | 스냅샷 |
| `positions` | 종목 | `[instrument]` | 수량 · 평단 · 매입금액 · 평가금액 · 평가손익(률) · 비중 | 계좌 · 시장 · 자산군 | `OPTIONAL` | 스냅샷 |
| `allocation` | 축 값 | 축 1개 택일 | 평가금액 · 비중 · 종목수 | 계좌 · 계좌유형 | `OPTIONAL` | 스냅샷 |
| `accounts` | 계좌 | `[account_type, account]` | 평가금액 · 예수금 · 평가손익(률) · 비중 | — | `NONE` | 스냅샷 |
| `realized-pnl` | 기간 × 종목 × 체결 | `[instrument, trade]` | 매도금액 · 취득원가 · 실현손익(률) | 계좌 · 기간 | `NONE` | 거래 |
| `asset-change` | 기간 × 유형 | — | — | 계좌 · 기간 | `NONE` | 스냅샷 + 현금흐름 |

정렬은 평가금액 내림차순 고정이며 요청 파라미터로 받지 않는다. 화면에서 정렬 기준을 바꾸는 것은 표시의 문제이므로 클라이언트가 받은 행을 다시 정렬한다(§2.5). `realized-pnl`과 `asset-change`는 공통 행 스키마를 쓰지 않으며 응답 형태는 §8.4가 정의한다.

`lens_policy`가 렌즈 노출을 한 곳에서 통제한다. 뷰 컴포넌트가 각자 판단하지 않는다. `OPTIONAL`인 뷰에서 렌즈를 켜면 `lens_safe`가 `TOTAL_ONLY`·`NEVER`인 지표가 행에서 빠진다.

요약은 `total`(렌즈 무관)과 자산구성 미니차트(렌즈 적용)를 함께 담으므로 뷰 전체가 아니라 미니차트 블록에만 렌즈가 붙는다. 총합이 보존되므로 렌즈를 켜도 `total`은 변하지 않는다.

### 6.4 서빙 계약

뷰 사양은 서버가 보유한다. 클라이언트는 `view_key`와 카탈로그에 정의된 파라미터만 보내며, 임의 조합을 받는 범용 쿼리 엔드포인트는 두지 않는다. **모든 뷰는 인증 주체의 계좌만 집계한다** — 스코프는 카탈로그 파라미터가 아니다(§3.8). **스냅샷 4개 뷰**는 `Axis.applicable_views`와 `View.lens_policy`가 곧 허용 파라미터 목록이므로 요청 검증이 카탈로그 대조로 끝난다. 실현손익·자산 변화는 원장이 달라 축·지표가 고정이므로(§1.4) `period`·`account`만 검증한다. 엔드포인트와 응답 스키마는 §8.

---

## 7. 계좌 연동 · 동기화

### 7.1 두 경로

| | KIS (본인 계좌) | CODEF (타 기관) |
|---|---|---|
| 입력 | 본인 앱키·시크릿·계좌번호 | 기관 선택 + 인증서 또는 ID/PW |
| 저장 | 시크릿 매니저에 보관하고 테이블에는 `credential_ref`만 | Connected ID를 시크릿 매니저에 보관 — 인증정보는 CODEF만 보유 |
| 소유 | 연동한 사용자의 계좌로 귀속 | 동일 |
| 전송 | — | 프론트에서 CODEF 공개키로 RSA 암호화 후 전송 |
| 특성 | 빠르고 안정적 | 스크래핑 기반 — 느리고 실패 가능 |
| 재시도 | 레이트리밋 스로틀 중심 | 지수 백오프 + 부분 실패 허용 |

1차에서는 자동 연동만 지원한다. 수동 입력·CSV 업로드는 제공하지 않으며, 연동 불가 계좌는 안내만 표시한다.

**자격증명은 사용자별로 격리된다.** 연동은 인증 주체의 계좌를 만드는 행위이므로 `account.user_id`가 그 자리에서 정해지고, `credential_ref`가 가리키는 시크릿 매니저 키 이름에도 사용자가 들어가 키 이름을 알아도 남의 것을 집을 수 없게 한다. 자격증명 값은 백엔드도 보관하지 않으며, 수집이 필요한 시점에 `collection_run` 픽업으로만 전달된다(§7.7 · §11.2).

**`account_ref`는 계좌가 특정된 시점에 발급한다.** `기관코드 + 계좌번호`를 소금과 함께 해시한 값이며, 계좌를 식별하는 유일한 안정적 입력이 그것이라 해제 후 재등록해도 같은 값이 나온다. 소금을 두는 이유는 계좌번호 공간이 좁아 해시만으로는 역산할 수 있어서다. `account_id`를 그대로 쓰지 않는 것은 재등록 때 새 uuid가 나와 과거 `cln_trade`와의 연결이 끊기기 때문이다 — `trade_id`가 이 값을 접두로 갖는다(§11.2).

**계좌 행은 계좌번호를 안 뒤에 만든다.** KIS는 계좌번호가 등록 입력에 있고, CODEF는 1차 인증 후 기관이 돌려준 계좌 목록에서 사용자가 고른 뒤 등록 요청이 온다. 두 경로 모두 행을 만드는 시점에 `account_ref`를 채울 수 있으므로 값이 빈 계좌가 생기지 않는다. 등록 처리 중 기관이 추가인증을 요구하면 계좌는 이미 특정된 상태이고 자격증명만 미완이므로, `REAUTH_REQUIRED`로 남겨 재인증 흐름에 태운다(§7.2 · §8.5).

### 7.2 계좌 상태

**연동 상태와 수집 결과는 직교한다.** 연동은 자격증명의 지속적 상태이고 수집 결과는 매일 갱신되는 이벤트다. 한 enum에 섞으면 어제 실패한 정상 연동 계좌를 표현할 수 없다.

`account.link_state` — 연동(자격증명) 상태만 담는다.

```
                  ┌──────────────┐
   연동 시작 ─────▶│  CONNECTING  │
                  └──────┬───────┘
                    성공 │    추가인증 요구
                         ▼         ▼
                  ┌───────────┐  ┌──────────────────┐
                  │ CONNECTED │◀▶│ REAUTH_REQUIRED  │◀── 인증 만료
                  └─────┬─────┘  └──────────────────┘
                        │  사용자 해제
                        ▼
                  ┌──────────────┐
                  │ DISCONNECTED │   과거 라인 보존
                  └──────────────┘
```

수집 결과는 저장하지 않고 **그 계좌의 최신 `collection_run`에서 파생**한다. 계좌별 뷰 응답은 두 값을 나란히 싣는다.

| 필드 | 출처 |
|---|---|
| `link_state` | `account.link_state` |
| `last_collection` | 최신 `collection_run`의 `state` · `as_of` · `failure_reason` |

`REAUTH_REQUIRED` 계좌도 EOD 배치가 `collection_run`을 만들되 즉시 같은 상태로 종료한다. 그러면 캐리포워드 트리거가 계좌 상태와 무관하게 **"그날 `collection_run` 최종 상태가 `DONE`이 아니면 이월"** 이라는 단일 규칙이 된다. `DISCONNECTED` 계좌는 `collection_run`을 만들지 않으며, 이것이 §7.5의 "제외"를 구현한다.

### 7.3 캐리포워드 — 실패해도 라인은 만든다

계좌 동기화가 실패해도 **그날의 `position_line`은 반드시 생성한다.** 직전 성공 스냅샷을 그날 `as_of`로 복제하고 `source_as_of`·`is_carried_forward`로 표시한다.

1. **그레인 유일성이 유지된다.** `as_of`마다 **연동이 유효한** 모든 계좌가 정확히 1행씩 있어야 총자산·비중이 성립한다. 빠뜨리면 그날만 총자산이 급락해 손실처럼 보인다. `DISCONNECTED` 계좌는 대상이 아니다. 판정 범위는 사용자 단위다 — 한 사용자의 계좌가 빠진 것이 다른 사용자의 화면을 흔들지 않는다(§3.8).
2. **자산 변화 뷰가 깨지지 않는다.** 기초·기말 스냅샷이 항상 존재해야 항등식이 성립한다.
3. **오차가 정직하게 흘러간다.** 이월된 평가금액은 낡은 시세이지만, 그 오차는 다음 성공 동기화 시 자산 변화 뷰의 `설명되지 않는 변화`로 드러난다.

### 7.4 as-of의 두 층

| 층 | 의미 | 노출 |
|---|---|---|
| **화면 as-of** | 배치 실행 기준일 | 모든 뷰 상단 배너 (단일) |
| **`source_as_of`** | 계좌별 실제 데이터 시각 | 계좌별 뷰, 지연 시 전역 배너 |
| **시장별 가격 기준일** | 종목이 속한 시장의 종가 날짜. `source_as_of` 기준 직전 해당 시장 영업일로 **유도한다** — 저장하지 않는다 | `PRICE_LAG_MARKET` notice |

세 번째 층은 한 계좌에 국내·미국 종목이 섞여 `source_as_of` 하나로 말할 수 없기 때문에 필요하다. 국내 영업일 캘린더는 `as_of` 생성 규칙(§9.1)에 이미 필요하므로, 시장 캘린더를 하나 더 두면 유도로 끝난다.

집계는 단일 as-of 기준으로 정렬하고, 계좌마다 동기화 시각이 다른 현실은 두 번째 층에서 표현한다.

### 7.5 부분 실패 시 표시 정책

| 상황 | 총자산 | 표시 |
|---|---|---|
| 일부 계좌 실패 | **포함**(이월값) | 계좌별 뷰 배지 `07-26 이월` + 탭바 점 |
| 재인증 필요 | 포함(이월값) | 계좌별 뷰 배지 + 재인증 CTA + 탭바 점 |
| 연동 해제 | **제외** | 자산 변화 뷰의 `－ 계좌 제외` 항목 |

계좌 단위 상태를 계좌별 뷰에 모으는 이유는 **조치와 표시가 같은 자리에 있어야 하기 때문**이다. 재인증도 해제도 그 화면에서 하고, 어느 계좌가 낡았는지는 계좌마다 다른 사실이라 전역 배너 한 줄로 말하면 어느 계좌인지 알 수 없다. 다른 화면을 보던 사용자에게는 탭바 점 하나로 알린다.

실패 계좌를 총자산에서 제외하면 자산이 급감해 더 큰 오해를 낳는다. **포함하되 낡았다고 말하는 쪽**이 안전하다. 연동 해제는 사용자가 의도한 행위이므로 제외하되, 손실로 오인되지 않게 자산 변화 뷰에서 별도 항목으로 분리한다.

연동 해제 시 과거 `position_line`은 보존한다(soft delete). 삭제하면 과거 기간의 자산 변화 계산이 불가능해진다.

### 7.6 동기화 트리거

| 트리거 | 범위 | 응답 |
|---|---|---|
| EOD 일배치 | 전 사용자의 전 계좌 | — |
| 수동 새로고침 | 인증 주체의 전 계좌 또는 단일 계좌 | `202` 즉시 반환 → 완료 시 푸시/폴링 |
| 재인증 완료 | 해당 계좌 | 즉시 트리거 |

동기화는 무겁고 느리므로 **항상 비동기**다. 새로고침은 즉시 응답하고 진행 상태만 표시한다.

사용자가 트리거하는 동기화는 본인 계좌만 대상으로 한다. 남의 계좌를 지정하면 거부하고(§9.3), `sync_run` 조회도 발급한 사용자에게만 열린다.

### 7.7 팀 경계 오케스트레이션

증권사 호출은 데이터가 하고 `position_line` 생성은 백엔드가 하므로(§11.1), 두 단계를 잇는 계약이 필요하다. 데이터 소유 `collection_run` 테이블 하나로 양방향을 처리한다.

| 컬럼 | 타입 | 비고 |
|---|---|---|
| `run_id` | uuid PK | |
| `account_ref` | text | |
| `requested_by` | enum | `SCHEDULE` · `USER` |
| `state` | enum | `REQUESTED` · `RUNNING` · `DONE` · `FAILED` · `REAUTH_REQUIRED` |
| `credential_ref` | text null | 픽업 시 전달되는 단기 참조 (§11.2) |
| `as_of` | date | 수집 대상 기준일자 |
| `requested_at` · `finished_at` | timestamptz | |
| `failure_reason` | text null | |

```
수동 새로고침   백엔드가 REQUESTED 행 삽입 → 데이터 잡이 집어감
EOD 배치       데이터 잡이 스스로 REQUESTED 행 생성
수집 종료      데이터가 cln_* 기록 후 DONE / FAILED / REAUTH_REQUIRED 로 전이
1단계 실행     백엔드가 계좌별 최종 상태를 읽어 진행
```

- **계좌 단위로 독립**이다. 한 계좌가 `FAILED`여도 나머지는 진행하고, 실패 계좌는 캐리포워드한다(§7.3).
- `REAUTH_REQUIRED`는 계좌 상태로 옮겨 재인증 흐름에 태운다(§7.2).
- 백엔드의 `sync_run`은 사용자에게 보여줄 진행 상태를 담고, `collection_run`을 참조한다.
- `position_line.as_of`는 실행 시각이 아니라 **`collection_run.as_of`를 그대로 따른다.** 수동 새로고침이 자정을 걸쳐 끝나도 요청 시 지정한 날짜에 기록된다.
- 큐 인프라 없이 테이블 하나로 트리거·완료·부분 실패가 모두 표현된다.

---

## 8. API 계약

### 8.1 엔드포인트

| 구분 | 경로 |
|---|---|
| 인증 | `POST /auth/login` · `GET /auth/me` |
| 뷰 | `GET /portfolio/views/summary` |
| | `GET /portfolio/views/positions?lens=&account=&market=&asset_class=` |
| | `GET /portfolio/views/allocation?axis=&lens=&account=&account_type=` |
| | `GET /portfolio/views/accounts` |
| | `GET /portfolio/views/realized-pnl?period=&account=` |
| | `GET /portfolio/views/asset-change?period=&account=` |
| 하위 화면 | `GET /portfolio/instruments/{id}` |
| 계좌 | `GET /accounts` · `POST /accounts` · `POST /accounts/{id}/reauth` · `DELETE /accounts/{id}` |
| 동기화 | `POST /sync` → `202` · `GET /sync/{run_id}` |
| 카탈로그 | `GET /portfolio/catalog` |

`POST /auth/login`을 뺀 전 엔드포인트가 인증을 요구하며, 응답 범위는 인증 주체의 계좌로 한정된다(§8.8). 경로에 사용자가 나타나지 않는 이유는 스코프가 토큰에서만 오기 때문이다 — `/users/{id}/portfolio/...` 꼴이면 남의 ID를 넣어볼 자리가 생긴다.

카탈로그 엔드포인트는 뷰별 허용 축·렌즈·필터 옵션과 연동 계좌 목록을 내린다. 계좌 목록은 인증 주체의 것만 담기므로 이 응답이 곧 허용 계좌 필터 값의 전부다. 허용 필터는 **렌즈 상태별로** 내려, `LOOK_THROUGH`에서 비활성화되는 필터(§9.3)를 클라이언트가 따로 판단하지 않게 한다. 클라이언트가 이 목록을 하드코딩하면 축 하나를 늘릴 때 앱 배포가 필요해져 서버가 뷰 사양을 갖는 의미가 사라진다.

### 8.2 응답 봉투

모든 뷰 응답이 같은 봉투를 쓴다.

```json
{
  "as_of": "2026-07-27T15:30:00+09:00",
  "data": { },
  "empty_reason": null,
  "notices": [
    { "code": "STALE_ACCOUNTS",
      "severity": "warn",
      "message": "1개 계좌가 07-26 기준입니다",
      "params": { "count": 1, "oldest": "2026-07-26" } }
  ]
}
```

`message`는 서버가 완성해 내리고 `code`를 함께 준다. 클라이언트는 `message`를 그대로 출력하고 `code`로 아이콘·색·후속 동작을 분기한다. 문구 수정에 앱 배포가 필요 없다.

`severity`는 `info | warn | error`.

| code | 발생 조건 | params |
|---|---|---|
| `FX_APPLIED` | 원화 환산에 적용한 환율 (§2.3) | 통화쌍별 배열 + 최고령 `fx_as_of`. 라인마다 환율이 달라 단일 값으로 표기할 수 없다 |
| `STALE_ACCOUNTS` | 캐리포워드된 계좌 존재 (§7.3) | `count` · 최고령 `source_as_of` |
| `CONSTITUENT_AS_OF` | 렌즈 적용 시 구성비중 기준일 (§3.4) | 최고령 기준일 + 대상 ETF 수. ETF마다 파일 갱신 주기가 달라 단일 날짜가 성립하지 않는다 |
| `CONSTITUENT_UNAVAILABLE` | 구성종목 미확보 ETF 존재 (§3.4) | `count` · 미분해 평가금액 |
| `LENS_METRICS_OMITTED` | `TOTAL_ONLY` 지표가 행에서 빠짐 (§6.2) | 생략된 지표 키 배열 |
| `EXCLUDED_ACCOUNTS` | 실현손익 합계에서 빠진 계좌 (§2.8) | `count` |
| `SEEDED_ROWS` | 추정 등급 행 존재 (§4.5) | `count` |
| `CA_UNKNOWN` | 기업행위 이력 미확인 (§4.4) | `instrument_id` 배열 |
| `CASHFLOW_UNCOVERED` | 현금흐름 미확보 계좌 (§4.6) | `count` |
| `PERIOD_TRUNCATED` | 기초 스냅샷 대체와 실제 시작일 (§2.9) | 실제 시작일 |
| `BOUNDARY_CARRIED_FORWARD` | 기간 경계 스냅샷에 이월 계좌 (§2.9) | `count` · 경계 시점 |
| `REAUTH_REQUIRED` | 재인증 대기 계좌 존재 (§7.2) | `count` |
| `SYNC_IN_PROGRESS` | 동기화 진행 중 | `sync_run_id` |
| `PRICE_LAG_MARKET` | 시장별 가격 기준일이 화면 `as_of`보다 이르다 (§5.4) | 시장별 가격 기준일 |
| `ALREADY_FINAL` | EOD 확정 후 또는 비영업일 수동 동기화 요청 | 확정 시각 |
| `FX_STALE` | 환율 폴백이 5영업일을 넘음 (§5.3) | 통화쌍 · `fx_as_of` |

**빈 상태는 notices가 아니라 봉투 필드로 표현한다.** `rows`가 비면 `empty_reason`이 필수다. 빈 상태마다 코드를 만들면 뷰가 늘 때마다 코드가 증식한다.

```
empty_reason : NO_ACCOUNTS · NO_HOLDINGS · NO_MATCH_FILTER
             · NO_TRADES_IN_PERIOD · ALL_UNAVAILABLE
```

**계좌 단위 상태는 notices에 넣지 않는다.** 전역 배너와 행 배지는 다른 물건이므로, 계좌별 뷰 응답의 행에 `link_state` · `last_collection`을 직접 싣는다(§7.2). 계좌별 뷰만 §8.3 공통 행 스키마를 확장하는 유일한 뷰다.

### 8.3 스냅샷 뷰 응답

요약·종목별·비중 분석·계좌별은 같은 팩트에 `group_by`만 다른 것이므로 응답 스키마도 하나다.

```json
{
  "group_by": ["sector"],
  "lens": "DIRECT",
  "total": { "total_assets_krw": 58000000, "securities_value_krw": 54300000,
             "deposit_krw": 3700000, "cost_amount_krw": 49200000,
             "unrealized_pnl_krw": 5100000, "unrealized_pnl_pct": 10.4,
             "cash_ratio_pct": 6.4 },
  "rows": [
    { "key": "semiconductor", "label": "반도체",
      "market_value_krw": 10614000, "cost_amount_krw": 9750000,
      "unrealized_pnl_krw": 864000, "unrealized_pnl_pct": 8.9,
      "weight_pct": 18.3, "instrument_count": 4 }
  ]
}
```

- **파생 지표를 서버가 계산해 포함한다.** 가산성 규칙(§1.5)이 서버 안에 갇혀 클라이언트가 비율을 잘못 평균낼 여지가 없다.
- `group_by`가 2단계면 `rows[].rows`로 중첩된다. 계좌별이 `["account_type","account"]`이며 **소계는 항상 서버가 계산한다.**
- 요약은 `group_by`가 비어 `total`만 채워지고, 미니차트는 하위 블록으로 실린다.

```json
{ "total": { "total_assets_krw": 58000000, "daily_change_krw": 1240000,
             "daily_change_pct": 2.2, "cash_ratio_pct": 6.4 },
  "mini_chart": { "group_by": ["market"], "lens": "LOOK_THROUGH",
                  "rows": [ { "key": "KR", "label": "국내주식",
                              "market_value_krw": 25520000, "weight_pct": 44.0 } ] } }
```
- 단일 통화 행에는 현지 통화 값을 함께 싣는다(§3.7).
- `Σ rows.market_value_krw = total.total_assets_krw`가 항상 성립한다. 손익 계열은 `securities_value_krw`를 기준으로 하므로 `securities_value_krw − cost_amount_krw = unrealized_pnl_krw`도 성립한다.
- `lens = LOOK_THROUGH`이면 `TOTAL_ONLY` 지표가 `rows[]`에서 빠지고 `total`에만 남으며 `LENS_METRICS_OMITTED` notice가 붙는다. 위 예시의 `rows[]`에서 `cost_amount_krw` · `unrealized_pnl_krw` · `unrealized_pnl_pct` 세 개가 사라진다.
- CASH 행의 원가·손익은 `null`로 내린다.

### 8.4 실현손익 · 자산 변화 응답

원장이 달라 스키마가 다르다.

```json
// realized-pnl
{ "period": { "from": "2026-01-01", "to": "2026-12-31" },
  "total": { "realized_pnl_krw": 2140000 },
  "rows": [
    { "key": "005930", "label": "삼성전자",
      "sell_amount_krw": 3560000, "cost_basis_krw": 2740000,
      "realized_pnl_krw": 820000, "realized_pnl_pct": 29.9,
      "first_sold_at": "2026-03-02", "last_sold_at": "2026-05-12",
      "trade_count": 3, "grade": "MIXED",
      "rows": [
        { "trade_id": "...", "sold_at": "2026-05-12", "quantity": 5,
          "realized_pnl_krw": 420000, "grade": "VERIFIED" } ] } ] }
```

```json
// asset-change
// breakdown[].type = DEPOSIT · WITHDRAW · DIVIDEND · FEE_TAX · INVESTMENT_PNL
//                    (표시 유형이며 cln_cashflow.type과 별개)
{ "period": { "from": "2026-07-01", "to": "2026-07-31" },
  "opening": 50000000, "closing": 58000000,
  "deposited": 6000000,
  "earned": 2000000,
  "account_included": 0, "account_excluded": 0,
  "breakdown": [ { "type": "DEPOSIT", "amount": 6000000 },
                 { "type": "DIVIDEND", "amount": 120000 },
                 { "type": "FEE_TAX", "amount": -30000 },
                 { "type": "INVESTMENT_PNL", "amount": 1910000 } ],
  "investment_pnl": { "total": 1910000, "realized": 850000,
                      "unrealized_change": 1060000, "split_available": true } }
```

실현손익 응답에서 **종목 노드의 `grade`는 4값에 `MIXED`를 더한 5값**이고 체결 노드는 4값이다. `MIXED`는 저장되지 않으며 응답 조립 시에만 생긴다.

`split_available`이 `false`이면 거래 원장이 없어 실현/미실현 분해를 생략한 것이다. `total`은 항상 정확하다(§2.9).

계좌 편입·제외는 §2.9가 별도 행으로 그리므로 `account_included` · `account_excluded` 두 최상위 필드로 둔다. `breakdown[]`에 넣지 않는다.

### 8.5 하위 화면 · 계좌 · 동기화 응답

```json
// GET /portfolio/instruments/{id}
{ "instrument": { "id": "...", "label": "삼성전자", "market": "KR", "currency": "KRW" },
  "position": { "quantity": 10, "avg_cost": 65610, "market_value_krw": 712000,
                "unrealized_pnl_krw": 55900, "unrealized_pnl_pct": 8.5 },
  "by_account": [ { "account_id": "...", "label": "한국투자 위탁",
                    "account_type": "GENERAL", "quantity": 7, "avg_cost": 64200 } ],
  "trades": [ { "executed_at": "2026-03-11", "side": "BUY", "quantity": 5, "price": 62000 } ],
  "basis": { "grade": "SEEDED", "coverage_start_at": "2026-01-01", "ca_unknown": true },
  "corporate_actions": [ { "type": "SPLIT", "ratio": 2, "ex_date": "2026-04-15" } ] }
```

```json
// GET /sync/{run_id}
{ "run_id": "...", "state": "RUNNING", "started_at": "...", "finished_at": null,
  "accounts": [ { "account_id": "...", "label": "한국투자 위탁",
                  "state": "DONE", "as_of": "2026-07-27", "failure_reason": null },
                { "account_id": "...", "label": "미래에셋증권",
                  "state": "FAILED", "as_of": "2026-07-26",
                  "failure_reason": "SCRAPE_TIMEOUT" } ] }
```

```json
// POST /accounts  → 202
{ "account_id": "...", "link_state": "REAUTH_REQUIRED",
  "challenge": { "type": "SMS_OTP", "expires_at": "...", "resend_available_at": "..." } }
```

`link_state`가 `CONNECTED`이면 `challenge`는 `null`이다.

### 8.6 오류와 빈 상태

- **빈 상태는 오류가 아니다.** `rows: []`와 `empty_reason`으로 표현하며 상태 코드는 `200`이다. §10의 빈 상태가 모두 여기 해당한다.
- 동기화 중이거나 스냅샷이 아직 없어도 `200`과 `notices`로 알린다.
- 재인증 필요는 오류가 아니라 계좌 상태다. `GET /accounts` 응답의 상태 필드와 `REAUTH_REQUIRED` notice로 표현한다. **인증 만료(`401`)와는 다른 층이다** — 하나는 증권사 자격증명, 하나는 우리 서비스 로그인이다.
- 오류는 `{ "error": { "code", "message" } }` 형태로 내리고 `code`를 클라이언트 분기용으로 쓴다.
- 인증·권한 실패는 `401` · `403`이며 봉투를 쓰지 않는다(§8.8). 데이터가 없는 것이 아니라 응답할 자격이 없는 것이므로 빈 상태와 구분한다.

### 8.7 UI ↔ 모델 매핑

| UI 요소 | 모델 경로 |
|---|---|
| 로그인 | `POST /auth/login` → access token |
| 화면에 보이는 자산의 범위 | 토큰 `sub` = `account.user_id` (§3.8) |
| as-of 배너 | 응답 봉투 `as_of` |
| 적용 환율 표기 | `FX_APPLIED` notice (`fx_rate` · `fx_as_of`) |
| 지연 배너 | `STALE_ACCOUNTS` notice (`is_carried_forward` 집계) |
| 새로고침 | `POST /sync` → `sync_run` |
| 총자산 | `Σ market_value_krw` (CASH 포함) |
| 총평가손익 | `Σ market_value − Σ cost_amount` (CASH 제외) |
| 일간 변화 | 당일 − 직전 `as_of`의 `total_assets_krw` |
| 비중 | `Σ market_value_krw ÷ 전체 Σ` |
| 현지 통화 병기 | 단일 통화 행의 `*_local` |
| 렌즈 토글 | `View.lens_policy = OPTIONAL` 인 뷰에서만 노출 |
| 구성비중 기준일 | `CONSTITUENT_AS_OF` notice |
| 계좌유형 소계 | `group_by = ["account_type","account"]`의 상위 노드 |
| 실현손익 행 | `realized_pnl_line` |
| 신뢰도 배지 | `position_basis.grade` → `realized_pnl_line.grade` |
| 미포함 계좌 N개 | `EXCLUDED_ACCOUNTS` notice |
| 자산 변화 항목 막대 | `breakdown[]` |
| 계좌 편입·제외 막대 | 최상위 `account_included` · `account_excluded` |
| 투자손익 | `Δ총자산 − 넣은 돈 − 배당 + 수수료 − 계좌 편입·제외` |
| 커버리지 경고 | `CASHFLOW_UNCOVERED` notice |

### 8.8 인증

사용자를 식별해야 데이터 범위가 갈리므로(§3.8) 인증은 선택이 아니다. 이메일·비밀번호로 로그인하고 **JWT access token을 Bearer 헤더**로 보낸다.

| 항목 | 값 |
|---|---|
| 토큰 | JWT, HS256 서명. 비밀키는 환경변수 |
| 실린 것 | `sub`(= `user_id`) · `iat` · `exp` |
| 전달 | `Authorization: Bearer <token>` |
| 만료 | 12시간. 만료되면 재로그인한다 |
| 서버 상태 | 없음 — 세션을 보관하지 않는다 |

**토큰에 계좌 목록을 싣지 않는다.** 계좌는 연동·해제로 바뀌는데 토큰은 발급 시점에 고정된다. 소유는 매 요청 DB에서 확인하며, 그래야 방금 해제한 계좌가 토큰 만료까지 조회되는 일이 없다.

```json
// POST /auth/login   { "email": "…", "password": "…" }
{ "access_token": "eyJ…", "expires_at": "2026-07-28T03:30:00+09:00" }

// GET /auth/me
{ "user_id": "…", "email": "…", "display_name": "yhr" }
```

**사용자 ID는 요청에서 받지 않는다.** 경로·쿼리·바디 어디에도 `user_id`가 없다. 두면 토큰과 파라미터 중 무엇을 신뢰할지의 문제가 생기고, 답이 하나뿐인 질문에 파라미터를 두는 셈이 된다.

| 상황 | 코드 | `error.code` |
|---|---|---|
| 토큰 없음 · 만료 · 서명 불일치 | `401` | `UNAUTHENTICATED` |
| 로그인 실패 | `401` | `INVALID_CREDENTIALS` |
| 남의 계좌를 필터·동기화 대상으로 지정 | `403` | `FORBIDDEN_ACCOUNT` |

로그인 실패는 **이메일이 없는 것과 비밀번호가 틀린 것을 구분하지 않는다.** 구분하면 가입 여부가 응답으로 새어 나간다.

**비밀번호는 BCrypt 해시만 저장한다.** 원문은 로그·응답 어디에도 남기지 않는다. 사용자 행은 마이그레이션이 심으며 회원가입·비밀번호 재설정 경로를 두지 않는다(§12).

---

## 9. 검증 규칙

강제 지점이 다른 세 묶음으로 나눈다. 첫 묶음만 저장·집계 전 앱 계층에서 실행 시점에 검사한다.

### 9.1 런타임 검증 — 저장·집계 전 통과

**사용자 스코프**

| 규칙 |
|---|
| 사용자 소유 테이블 조회는 인증 주체의 `user_id`로 스코프된다. 스코프 없이 부를 수 있는 조회 경로를 두지 않는다 |
| 스코프는 렌즈 전개(§3.6 2단계) **앞**에서 걸린다. 이후 단계가 스코프를 다시 걸 필요가 없다 |
| `as_of` 후보는 그 사용자의 계좌에 라인이 있는 날짜뿐 (§3.8) |
| `account.user_id`는 생성 후 변경 불가 |
| `account.account_ref`는 생성 후 변경 불가. 해제한 계좌를 다시 연동하면 같은 값을 재사용한다 (§7.1) |

**팩트 정합성**

| 규칙 |
|---|
| `position_line`은 (as_of, account, instrument)마다 **정확히 1행** — 그레인 유일성 |
| `market_value_krw`가 있으면 `fx_rate`·`fx_as_of` 필수 (null 금지) |
| 한 사용자의 연동이 유효한(`DISCONNECTED`가 아닌) 모든 계좌는 그 사용자의 해당 `as_of`에 라인 존재 |
| 그날 `collection_run` 최종 상태가 `DONE`이 아니면 캐리포워드 |
| `is_carried_forward = true`이면 `source_as_of < as_of` |
| `as_of`는 국내 증시 영업일에만 생성. 당일 외의 `as_of` 행은 수정 불가 |
| 비영업일 `collection_run`은 라인을 만들지 않는다 |
| `is_final = true`인 행은 수동 동기화가 덮지 않는다 |
| `fx_rate` 결측 시 직전 영업일로 폴백하고 `fx_as_of`에 그 날짜를 기록 |
| `position_line.cost_amount`는 잔고 평단 기준. `position_basis`를 참조하지 않는다 |
| CASH 행은 원가 = 평가금액으로 생성(§5.2) |

**평단 · 실현손익 · 등급**

| 규칙 |
|---|
| 등급 판정은 `corporate_action` 반영 **후**의 수량으로 계산 |
| 등급 판정의 좌변 수량은 `position_line.quantity`. 재구성 수량을 쓰지 않는다 |
| 등급 판정은 `position_line.as_of` 기준으로 정렬된 두 원장으로만 수행. 역산 체결은 `executed_at <= as_of` |
| `SEEDED`이면 `seed_avg_price` 필수 |
| `ca_unknown`은 `ca_coverage` 대조로 판정 |
| 확보 구간이 계좌 최초 체결일까지 닿았는데 `opening_qty > 0`이면 영구 `SEEDED` |
| 보유수량이 0이 되면 `position_basis` 리셋 |
| `realized_pnl_line`은 `trade_id` upsert로만 생성 (insert-only 금지) |
| `realized_pnl_line.grade`는 **산출 시점의 스냅샷**이며 이후 갱신하지 않는다 |
| 실현손익의 기간 귀속은 **체결일** 기준 |
| 외화 실현손익은 매입일·매도일 환율을 각각 적용 |

**렌즈**

| 규칙 |
|---|
| look-through 전개 후 `Σ market_value`가 전개 전과 일치 (총합 보존, 기타 버킷 포함) |
| `etf_coverage.state = UNAVAILABLE`인 ETF는 전개하지 않고 ETF 행을 남긴다 |

**자산 변화**

| 규칙 |
|---|
| 기간 양 끝의 `position_line` 필요. 기초가 없으면 가장 이른 스냅샷으로 대체하고 실제 시작일 표시 |
| 투자손익은 나머지 전부로 정의 — 별도 잔차 항목을 두지 않는다 |
| 계좌 편입·제외는 손익이 아닌 별도 항목. "넣은 돈"·"번 돈" 어디에도 넣지 않는다 |
| 배당·수수료·세금은 "번 돈"에 귀속 |
| `넣은 돈`은 `manual_cashflow`에서만 온다. `cln_cashflow`에 `DEPOSIT`·`WITHDRAW`가 있으면 거부 |

### 9.2 스키마 · 코드 규약 — 마이그레이션과 코드 리뷰에서 강제

| 규칙 | 강제 지점 |
|---|---|
| 비율 성격 컬럼(수익률·비중)은 스키마에 존재 불가 | 마이그레이션 리뷰 |
| `additive = false` 지표는 라인 단위 합산 금지 — 집계 후에만 계산 | 코드 리뷰 |

### 9.3 요청 검증과 응답 조립

**요청**

| 규칙 |
|---|
| `POST /auth/login`을 뺀 모든 요청은 유효한 토큰을 요구한다. 없거나 만료면 `401` (§8.8) |
| 사용자 ID를 요청 파라미터로 받지 않는다. 스코프는 토큰에서만 온다 |
| 계좌 필터·동기화 대상 계좌는 인증 주체 소유인지 검사한다. 아니면 빈 결과가 아니라 `403` |
| `view_key` · `axis` · `lens` · 필터 값은 카탈로그(§6)에 정의된 것만 허용 (스냅샷 4개 뷰) |
| 비활성 축(`is_leveraged` 등)은 요청 시 거부 |
| `lens = LOOK_THROUGH`이면 `lens_sensitive = true` 축의 필터를 거부. 계좌 계열만 허용 |

**응답**

| 규칙 |
|---|
| `lens = LOOK_THROUGH`이면 `lens_safe = NEVER` 지표를 제외, `TOTAL_ONLY` 지표를 `rows[]`에서만 제외 |
| 렌즈 적용 응답은 `CONSTITUENT_AS_OF` notice를 함께 싣는다 |
| 현지 통화 표시는 묶음이 단일 통화일 때만 허용. 그 외 집계 값은 원화(§3.7) |
| 비중의 분모는 `total_assets_krw`, 손익률의 분모는 `cost_amount_krw`(§6.2) |
| `UNAVAILABLE`·`CONFLICT` 종목은 실현손익 합계에서 제외하되 제외 건수를 노출 |
| 현금흐름 미확보 계좌, 기간 경계 캐리포워드 계좌는 notice로 표시 |
| `rows`가 비면 `empty_reason` 필수(§8.2) |

`etf_constituent`의 재귀 깊이 제한·순환 참조 차단과 `cln_cashflow`의 매매대금 배제는 **데이터 제공 계약**이며 백엔드에서 판정하지 않는다(§11.2).

---

## 10. 화면 상태 전수 (State Matrix)

와이어플로우는 해피패스만 시각화한다. 아래는 화면별 구현 대상 상태 전수.

| 화면 | 상태 |
|---|---|
| **로그인** | default / 입력 누락 / **인증 실패**(이메일·비밀번호 구분 없음) / 로딩 / **세션 만료로 되돌아옴** |
| 요약 | default / **연동 계좌 없음(온보딩)** / 로딩 / **일간 변화 없음**(연동 첫날) |
| 종목별 | default / **보유 종목 없음** / 필터 결과 없음 / **렌즈 ON — 2줄이 환산수량만 남고 시장·자산군 필터 비활성** |
| 비중 분석 | default / 축 전환 / 렌즈 ON(기타 버킷·구성비중 기준일 표기) / **구성종목 데이터 없음** / 섹터 축 미분류 안내 |
| 계좌별 | default / **재인증 필요 배지** / **캐리포워드 배지**(`07-24 기준`) / 그룹 접힘 / 연동 해제 확인 |
| 실현손익 | default / **해당 기간 매도 없음** / 추정 배지 / **등급 혼합(MIXED)** / **미포함 계좌 N개** / 전 계좌 UNAVAILABLE / 체결 펼침 / 지정 기간 입력 |
| 자산 변화 | default / **기초 스냅샷 없음 → 실제 시작일 표시** / **계좌 편입·제외** / **현금흐름 미확보 경고** / 기간 경계 이월 경고 / **손실 케이스** / 실현·미실현 분해 생략 |
| 종목 상세 | default / 거래내역 없음 / 기업행위 경고(`ca_unknown`) |
| 계좌 연동 | 기관 선택 / 인증 입력 / **추가 인증 요구** / 연동 실패·재시도 / 연동 성공 |
| 동기화 | 진행 중 / **부분 실패** / 전체 실패 / 완료 |

빈 상태는 `empty_reason`, 품질 경고는 `notices`, 계좌 단위 상태는 계좌별 뷰의 행 필드로 표현한다(§8.2).

**인증 만료는 화면별 상태가 아니다.** `401`은 어느 화면에서든 같은 뜻이므로 화면마다 상태를 두지 않고 로그인으로 되돌리는 전역 처리로 다룬다. 화면별 상태로 두면 아홉 곳에 같은 분기가 생긴다.

새 사용자는 계좌가 하나도 없는 상태로 시작하므로 요약 뷰의 온보딩(`empty_reason = NO_ACCOUNTS`)이 로그인 직후의 기본 상태다.

> 뒤로·취소 이동은 각 화면 네비바의 `‹`로 일관 제공한다.

---

## 11. 역할 분담

### 11.1 영역별

| 영역 | 데이터 | 백엔드 |
|---|---|---|
| 수집 | 증권사 잔고·거래·입출금 원본 적재, ETF 구성종목 파일, 환율 | — |
| 마스터 | 종목 마스터(ISIN·섹터·시장·통화·자산군) 소유·갱신, **ETF 구성비중 중첩 평탄화** | 조인만 |
| 도메인 계산 | — | `position_line` 생성(평단·기업행위·환산·캐리포워드), 등급 판정, 실현손익 |
| 조회 | — | 렌즈 전개 · 집계 · 파생 · 응답 조립 (§3.6) |
| 인증 | — | 로그인·토큰 발급과 검증, 사용자 스코프 강제 (§3.8 · §8.8) |
| 연동 | — | 계좌 연동·재인증·해제, 동기화 오케스트레이션 |
| 운영 | 수집 파이프라인 모니터링, 원천 장애 대응 | API·배치 잡 모니터링 |

look-through는 두 팀에 걸친다. **중첩 ETF를 펼쳐 `ETF → 최종 종목 → 최종 비중` 한 겹으로 만드는 것까지가 데이터**이고, 그 비중으로 사용자 보유분을 안분해 기타 버킷·총합 검증까지 하는 것이 **백엔드**다. 무겁고 자주 바뀌는 참조 데이터 손질과, 사용자 자산에 대한 도메인 규칙을 나눈 것이다.

인프라는 배포·클러스터·저장소·시크릿 관리를 담당하며 이 기능의 산출물에 직접 관여하지 않는다.

### 11.2 팀 경계 인터페이스

| 산출물 | 제공 → 소비 | 합의가 필요한 것 |
|---|---|---|
| 종목 마스터 | 데이터 → 백엔드 | 스키마 · 갱신 주기 · ISIN 미매칭 처리 |
| ETF 구성비중(평탄화) | 데이터 → 백엔드 | 스키마 · `as_of` · 비중 합 보장 여부 · 깊이 제한 · 순환 참조 차단 |
| 환율(일별) | 데이터 → 백엔드 | 통화쌍 · 소수 자릿수 · 결측 처리 |
| 원본 잔고·거래·입출금 | 데이터 → 백엔드 | 정규화 수준 · 중복 처리 · 조회 기간 한계 |
| `collection_run` | 양방향 | 상태 전이 규칙 · 픽업 주기 · 재시도 정책 (§7.7) |
| 자격증명 접근 | 백엔드 → 데이터 | 테이블 직접 접근을 열지 않는다. `collection_run` 픽업 시 단기 자격증명 참조를 함께 넘기며 인프라의 시크릿 관리에 얹는다 |
| `account_ref` 발급 | 백엔드 → 데이터 | 계좌 등록 시 백엔드가 발급하는 **불투명 문자열**(계좌번호 아님). `account.account_ref`에 저장하며(§5.1), 재연동해도 같은 계좌면 같은 값을 유지한다(§7.1). 데이터는 이 컬럼을 계좌 마스터에 그대로 받아 API 갈래(`cln_*`)와 대사할 수 있게 한다 |
| 사용자 개념 | 백엔드 전용 | 데이터팀 산출물에 `user_id`가 없다. 소유는 `account_ref` → 계좌 → 사용자로 백엔드가 해석한다 |
| `trade_id` 안정성 | 데이터 → 백엔드 | 같은 체결은 재수집·재처리에도 같은 값을 갖는다. 원천이 고유 ID를 주지 않으면 합성 규칙을 데이터가 문서화하고 고정한다 |
| 커버리지 | 데이터 → 백엔드 | `ca_coverage` · `etf_coverage`로 "없음"과 "미확보"를 구분해 제공 |
| 현금흐름 배제 규칙 | 데이터 → 백엔드 | 매매대금 · 예수금 내부 이동 · 환전 · 매매 수수료를 제외한 결과만 제공. 유형별 확보 가능 범위도 함께 (§5.1) |

테이블 소유는 §5.1을 따른다. 데이터는 `raw_*` · `cln_*` · `collection_run` · `instrument` · `etf_constituent` · `fx_rate` · `corporate_action` · `ca_coverage` · `etf_coverage`를, 백엔드는 `app_user` · `account` · `position_line` · `position_basis` · `realized_pnl_line` · `manual_cashflow` · `sync_run`을 소유한다.

**사용자는 백엔드 안에만 있다.** 데이터팀은 `account_ref` 단위로 수집하고 그 계좌가 누구 것인지 알 필요가 없다. 소유를 경계 너머로 넘기면 두 팀이 같은 사실을 각자 들고 있게 되고, 스코프가 어느 쪽에서 걸리는지가 흐려진다.

---

## 12. 미결 / 향후 확장 (YAGNI 경계)

초기 범위에서 **의도적으로 제외**한 것들.

- **회원가입과 비밀번호 재설정**: 사용자 행은 마이그레이션이 심는다(§5.1). 사용자가 늘어나는 속도가 사람을 추가하는 속도와 같아 셀프서비스 경로가 값을 하지 않는다. 열면 중복 이메일·비밀번호 정책·메일 발송이 함께 따라온다.
- **refresh 토큰과 강제 로그아웃**: access token 하나로 끝내고 만료되면 재로그인한다. 서버가 세션을 들지 않는 대가이며, 즉시 무효화가 필요해지는 시점 — 계정 탈취나 외부 공개 — 에 도입한다.
- **소셜 로그인(OIDC)**: 비밀번호를 저장하지 않는 이점이 있으나 외부 클라이언트 등록과 리다이렉트 흐름이 붙는다.
- **권한 등급과 계정 공유**: 모든 사용자가 같은 권한이고 자기 데이터만 본다. 가족 계정·읽기 전용 공유·관리자 역할을 두지 않는다. 공유를 열면 소유가 계좌 하나로 결정된다는 전제(§3.8)가 깨져 접근 판정이 별도 모델이 된다.
- **계좌 소유자 이전**: `account.user_id`는 생성 후 바뀌지 않는다. 옮기면 과거 스냅샷의 소유까지 함께 움직여 지난 기간의 자산 변화가 재현되지 않는다.
- **사용자 데이터 반출·일괄 삭제**: 계좌 해제는 있으나 계정 삭제는 없다.
- **벤치마크 비교**: KOSPI·S&P500 대비 상대 성과. 원천은 FinanceDataReader로 확보 가능하나 우선순위가 아니다.
- **섹터 내 PER/PBR 분위수**: 데이터 원천 미확정.
- **보유·거래내역의 수동 입력·CSV 업로드**: 자동 연동만 지원한다. 종목 수 × 거래 횟수만큼 입력이 필요하고 자동 연동이라는 대체재가 있어서다. 입출금은 예외로 사용자 입력을 받는다(§4.6) — 원천이 없어 대체재가 없고 빈도가 월 1~2건이다. 보유·거래내역에는 보정 경로가 없으므로 구조적 원인의 `CONFLICT`는 `UNAVAILABLE`로 강등해 경고를 영구 표시하지 않는다(§4.4).
- **기간수익률(TWR·MWR)**: 입출금 타이밍을 보정한 수익률로 절대금액 손익이 대체하지 못한다. 기초·기말 스냅샷과 기간 현금흐름이 있어 **계산 입력은 이미 확보돼 있으나** 우선순위가 아니다.
- **배당의 종목별 귀속**: 계좌 단위 현금흐름으로만 집계한다. 자산 변화의 배당 행은 제공하되 어느 종목에서 나왔는지는 따지지 않는다.
- **포트폴리오 스냅샷 비교·백테스트**: 후속 과제.
- **과거 시점 화면 재현**: 전개 시 어느 `as_of`의 구성비중을 썼는지 기록하지 않고 조회 시 항상 최신을 쓰므로, 과거 날짜의 분해를 그때 보였던 값으로 재현하지 않는다.
- **미국 종가의 당일 반영**: 한국 EOD 시점에 미국 시장이 열리지 않았으므로 직전 미국 영업일 종가를 쓰고 나중에 소급하지 않는다(§5.4).
- **미실현 평가손익의 환손익 분리**: 미실현 평가손익은 양변을 같은 환율로 환산해 환손익이 애초에 들어가지 않는다(§5.3). 실현손익은 매입일·매도일 환율을 각각 적용해 환손익을 포함하며(§4.3), 이를 가격손익과 분리하지 않는다.
- **사전 집계**: 보유 규모상 조회 시 집계로 충분하므로 집계 캐시나 전개 결과 저장을 두지 않는다(§3.3 · §3.4).
- **범용 쿼리 엔드포인트**: 뷰 사양은 서버가 보유하며 임의 축 조합을 받지 않는다(§6.4).
- **계좌 간 이체 식별**: 이체를 입출금과 구분하지 않는다. 계좌 필터 적용 시 각주로 고지한다(§2.9).
- **세무 신고용 손익**: 평단을 계좌마다 독립으로 계산하므로 인별 합산인 양도소득세와 일치하지 않는다(§4.3).

### 외부 의존 대기

| 항목 | 내용 |
|---|---|
| 국내 ETF 구성종목 원천 | 확보를 전제로 설계했다. 미확보 시 비중 분석의 `LOOK_THROUGH`가 국내 ETF에 한해 동작 불가 |
| 섹터 체계 통일 매핑 | 미국·국내 분류 체계 통일. 마스터 조인 방식이라 보정이 과거에 소급 반영됨(§3.3) |
| 기업행위 데이터 원천 | 미확보 시 `ca_unknown` 플래그가 광범위하게 표시됨 |
| 레버리지 구분 | 원천 확보 시 `instrument.is_leveraged` 축 활성화 |
| 종목 마스터 인터페이스 | 스키마·갱신 주기·미매칭 처리 규칙 합의 (§11.2) |
| ETF 구성비중 평탄화 제공 | 중첩 분해를 데이터 측에서 처리하는 형태 합의 (§11.2) |
| 알림 엔진과의 접점 | 알림 규칙이 평가손익률·비중을 읽는 인터페이스는 알림 스펙에서 정의 |

---

## 13. 다음 단계

1. **스펙 검증** — 결정 간 충돌과 누락 점검
2. **팀 경계 합의** — §11.2의 네 가지 인터페이스
3. **구현 계획** — 스키마·마이그레이션 → 동기화·캐리포워드 → 집계 레이어 → 렌즈 → 등급 판정 → 6개 뷰 API → UI → 상태 화면
4. **데이터 수집 설계** — 별도 스펙

구현 계획 단계에서 확정하는 것: 인덱스와 제약조건, 마이그레이션 순서, 스냅샷 보관 기간과 파티셔닝 시점, 토큰 비밀키 관리와 만료 시각의 운영값.
