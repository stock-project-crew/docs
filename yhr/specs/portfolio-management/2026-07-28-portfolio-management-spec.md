...
- **as-of 배너**: `기준 2026-07-27 15:30 · 새로고침`. 배치 실행 기준일자를 표기하고, 새로고침은 **그 화면의 뷰를 다시 조회한다.** 필터·축·렌즈 상태는 유지된다. 수동 동기화(§7.6)는 계좌 단위 행위이므로 계좌별 화면에서 트리거한다. 자산 변화는 기초·기말 두 시점을 함께 표기하므로 하단에 둔다.
...
| 컬럼 | 종목 · 매도일(`05-12 외 2건`) · 매도금액 · 취득원가 · 실현손익(금액/%) · **신뢰도 배지** |
| 펼침 | 종목 행을 탭하면 매도 체결이 펼쳐진다 |
...
**체결을 종목으로 접지 않는다.** 등급은 체결의 속성이고 `SEEDED` 배지를 탭하면 확보 구간 시작일과 시드 평단 출처를 보여줘야 하는데(§4.5), 종목으로 접으면 그 정보가 사라지고 "섞이면 낮은 쪽" 같은 임의 규칙이 생긴다. 대신 §8.3의 2단계 중첩(`rows[].rows`)을 그대로 써서 화면에서 종목별로 묶는다. 종목 노드의 `grade`는 체결 등급이 하나면 그 값, 섞이면 `MIXED`이며 이는 접기 결과가 아니라 펼쳐 보라는 신호다.
...
**"넣은 돈"은 사용자 입력에서 온다.** 증권사 입출금 이력에 기대지 않으므로(§4.6) 입력이 없는 기간은 그만큼 투자손익에 섞인다. 워터폴의 `넣은 돈` 행에서 바로 입력 화면으로 갈 수 있게 한다.
...
| 입출금 미입력 기간 | 경고 표시(§4.6). 그만큼 투자손익에 섞인다. 워터폴에서 입력 화면으로 유도 |
| 값이 0인 항목 | 행을 숨긴다 |
...
| 계좌별 | default / **재인증 필요 배지** / **캐리포워드 배지**(`07-26 이월`) / 동기화 실패 배지 / 연동 해제 확인 |
| 실현손익 | default / **해당 기간 매도 없음** / 추정 배지 / **등급 혼합(MIXED)** / **미포함 계좌 N개** / 전 계좌 UNAVAILABLE |
