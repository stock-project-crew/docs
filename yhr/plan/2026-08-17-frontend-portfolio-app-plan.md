...
├────────────────────────────────────────────┤
│ 삼성전자 [국내]              14,240,000원 │  Body 16
│ 24.6% · 200주 · 평단 60,000 +2,240,000 (+18.7%) │  Caption 12 / Body 14
├────────────────────────────────────────────┤
│ TIGER 미국나스닥100 [국내]   11,000,000원 │
│ 19.0% · 100주 · 평단 100,000 +1,000,000 (+10.0%) │
├────────────────────────────────────────────┤
│ 애플 [미국]           6,160,000원 ($4,400) │
│ 10.6% · 20주 · 평단 $200.00  +560,000 (+10.0%) │
├────────────────────────────────────────────┤
│ KRW 예수금                    4,560,000원 │
│ 7.9%                                    — │
└────────────────────────────────────────────┘
...
| 1줄 | 종목명 · 시장 배지 · 평가금액(+현지통화) |
| 2줄 | 비중 · 수량 · 평단 / 평가손익(금액·률) |
| 빼는 것 | 현재가(지표 카탈로그에 없음) · 매입금액(= 평가금액 − 평가손익) |
| 예수금 행 | **남긴다.** 빼면 비중 합이 100%가 안 되고 합계가 `total_assets_krw`와 어긋난다. 2줄은 비중만 남고 손익 자리가 `—` |
...
| 렌즈 ON | 2줄이 `19.0% · 환산 3.37주`로 줄어든다. 시장·자산군 필터 비활성 + 선택값 리셋. 손익 정렬 2종 비활성 |
| 렌즈 토글 이름 | `ETF 분해 모드` |
...
**비중은 1줄이 아니라 2줄 왼쪽에 둔다.** 1줄에 함께 두면 금액 블록이 넓어져(iPhone SE에서 148pt → 91pt 차이) 종목명을 눌러 버린다. `TIGER 미국나스닥100`의 끝 한 글자가 잘리면 다른 상품처럼 읽힌다.

**종목명 말줄임**: 최대 폭 205pt, 초과 시 뒤쪽 `…`. 1줄 고정. 폭 우선순위는 `금액 블록 > 시장 배지 > 종목명`이다. 실측 결과 iPhone SE(375pt)와 iPhone 14(390pt) 모두에서 표본 8행이 잘리지 않는다 — 가장 긴 `TIGER 미국나스닥100`이 145pt다.
...
- 비중 분석 · 계좌별 · 실현손익 · 자산 변화 네 화면 (설계 논의 후 이 문서에 Task 9~12로 추가)
